/**
 * LitRes Downloader v17.0
 * Рабочий поиск toc.js + все фичи
 * (c) 2026 Diminssoft
 */

(function fullDownloaderV17() {
    console.log('🚀 LitRes Downloader v17.0 — Рабочий toc.js + все фичи!');
    ...
    async function downloadLoop() {
        ...
        if (!success) {
            ...
            if (state.consecutiveErrors >= 20) {
                setStatus(`📌 ${state.consecutiveErrors} ошибок подряд! Останавливаем.`, true);
                await saveProgress();
                state.isRunning = false;
                updateButtons();
                return;
    ...
    async function downloadAndAddToZip(pageNum) {
        ...
        // Пробуем форматы: сначала gif (основной), потом jpg
        for (const ext of ['gif', 'jpg']) {
            try {
                addLog(`📥 Стр. ${pageNum} (${ext})...`);
                ...
                if (!imageUrl) {
                    addLog(`⚠️ Нет ссылки для стр. ${pageNum} (${ext})`, true);
                    continue;
                }
                ...
                try {
                    ...
                    if (!check.ok) {
                        addLog(`⚠️ Ссылка ${ext} не доступна: ${check.status}`, true);
                        continue;
                    }
                } catch(e) {
                    addLog(`⚠️ Ошибка проверки ${ext}: ${e.message}`, true);
                    continue;
                }
                ...
                if (!imgBlob) {
                    addLog(`⚠️ Не удалось получить изображение стр. ${pageNum} (${ext})`, true);
                    continue;
                }
                ...
                addLog(`⚠️ Ошибка стр. ${pageNum} (${ext}): ${e.message}`, true);
    ...
    console.log(`✅ LitRes Downloader v17.0 загружен!`);
