/**
 * LitRes Downloader v6.0
 * С диагностикой прямого скачивания + сохранение прогресса
 * (c) 2026 Diminssoft
 */

(function fullDownloaderV6() {
    console.log('🚀 LitRes Downloader v6.0 — Прямая загрузка');
    ...
    // ============================================================
    // 7. ДИАГНОСТИКА ПРЯМОГО СКАЧИВАНИЯ
    // ============================================================
    async function tryDirectDownload(artId, fileId) {
        try {
            ...
            for (const endpoint of endpoints) {
                const resp = await fetch(
                    `https://api.litres.ru/foundation/api/arts/files/${fileId}/link?${endpoint}&is_trial=false`,
                    {
                        method: 'GET',
                        credentials: 'include',
                        headers: getHeaders()
                ...
                const downloadUrl = data?.payload?.data?.link || 
                                   data?.payload?.link || 
                                   data?.data?.link ||
                                   data?.link;

                if (downloadUrl && (downloadUrl.includes('.zip') || downloadUrl.includes('.fb2') || downloadUrl.includes('.epub'))) {
                    return downloadUrl;
                }
            }
            return null;
        } catch(e) {
            console.warn('⚠️ Ошибка проверки прямого скачивания:', e);
            return null;
        }
    }
    ...
    async function directDownload(url) {
        try {
            addLog(`📥 Скачиваем книгу целиком...`);
            setStatus('📥 Скачивание полной книги...');
            ...
            let ext = 'zip';
            if (url.includes('.fb2')) ext = 'fb2';
            else if (url.includes('.epub')) ext = 'epub';
            else if (url.includes('.pdf')) ext = 'pdf';
            
            const fileName = `${bookTitle}.${ext}`;
            ...
            setStatus(`🎉 Книга скачана! ${fileName} (${Math.round(blob.size / 1024 / 1024)} MB)`);
            addLog(`✅ Книга скачана: ${fileName} (${Math.round(blob.size / 1024 / 1024)} MB)`);
            return true;

        } catch(e) {
            addLog(`❌ Ошибка прямого скачивания: ${e.message}`, true);
            return false;
        }
    }
    ...
    // ============================================================
    // 13. ОСНОВНАЯ ФУНКЦИЯ СКАЧИВАНИЯ
    // ============================================================
    ...
    async function startDownload() {
        ...
        // ============================================================
        // ДИАГНОСТИКА ПРЯМОГО СКАЧИВАНИЯ
        // ============================================================
        addLog('🔍 Проверяем возможность прямого скачивания...');
        setStatus('🔍 Проверка прямого скачивания...');
        
        const directUrl = await tryDirectDownload(state.artId, state.fileId);
        if (directUrl) {
            const confirmDirect = confirm(
                `📚 Найдена возможность скачать книгу целиком!\n\n` +
                `📖 ${state.bookTitle}\n` +
                `📦 Формат: ZIP-архив\n\n` +
                `✅ Это быстрее и проще, чем постраничная загрузка.\n` +
                `❌ Не получится собрать ZIP с картинками.\n\n` +
                `Скачать всю книгу сразу?`
            );
            if (confirmDirect) {
                const success = await directDownload(directUrl);
                if (success) {
                    state.isRunning = false;
                    updateButtons();
                    return;
                }
                addLog('⚠️ Прямое скачивание не удалось, переходим к постраничному...');
            } else {
                addLog('📌 Пользователь выбрал постраничную загрузку');
            }
        } else {
            addLog('ℹ️ Прямое скачивание недоступно');
        }
    ...
    console.log(`✅ LitRes Downloader v6.0 загружен!`);
