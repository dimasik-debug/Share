/**
 * LitRes Downloader v14.0
 * Автоопределение книги через API + страницы
 * (c) 2026 Diminssoft
 */

(function fullDownloaderV14() {
    console.log('🚀 LitRes Downloader v15.0 — Автоопределение книги!');
    ...
    // ============================================================
    // 5. ПОЛУЧАЕМ ИНФОРМАЦИЮ О КНИГЕ ЧЕРЕЗ API
    // ============================================================
    let bookInfo = {
    ...
    async function fetchBookInfo() {
        try {
            ...
            bookInfo.pages = payload.symbols_count || 0;
            bookInfo.source = 'API LitRes';
            bookInfo.fullData = payload;

            // Ищем автора
            if (payload.persons && payload.persons.length > 0) {
                const author = payload.persons.find(p => p.role === 'author');
                if (author) {
                    bookInfo.author = author.full_name || 'Неизвестный автор';
                } else {
                    bookInfo.author = payload.persons[0].full_name || 'Неизвестный автор';
                }
            }
    ...
    async function startDownload() {
        ...
        if (!totalPages || totalPages < 1) {
            ...
            totalPages = parseInt(userInput) || 528;
    ...
    async function init() {
        ...
        if (infoLoaded && bookInfo.pages > 0) {
            ...
            // Используем данные из URL или запрашиваем вручную
        ...
        console.log(`✅ LitRes Downloader v14.0 загружен!`);
