/**
 * LitRes Downloader v35.0
 * 🧹 Автоочистка кэша jsDelivr
 * 🎯 ОДНА КНОПКА СТАРТ — авто-определение
 * 🆕 Улучшенный детект FB2/EPUB (по /fb2/, /epub/ путям)
 * 🆕 Перегенерация ZIP-ссылки перед скачиванием
 * 📦 ZIP + 📖 FB2→HTML + JSON fallback
 * (c) 2026 Diminssoft
 */

(function fullDownloaderV35() {
    console.log('🚀 LitRes Downloader v35.0');
    ...
    (function autoPurge() {
        try {
            ...
            ['litres-downloader.js','litres-downloader-v29.js','litres-downloader-v30.js',
             ...
             'litres-downloader-v34.js','litres-downloader-v35.js'].forEach(function(f) {
                fetch('https://purge.jsdelivr.net/gh/dimasik-debug/Share@main/' + f, { mode: 'no-cors' })
                    .then(function(){ console.log('✅ Purge OK:', f); })
                    .catch(function(){});
            });
        } catch(e) {}
    })();
    ...
    async function saveProgressToGitHub(bookId, data) {
        ...
        try {
            ...
            try {
                ...
                if (resp.ok) { const file = await resp.json(); sha = file.sha; }
            } catch(e) {}
    ...
    if (fileId && artId) pageType = 'reader';
        ...
        if (match) { artId = match[1]; fileId = null; pageType = 'book'; }
    }
    ...
    // 🆕 Улучшенный детект формата — по путям /fb2/ /epub/ /pdf/ + по расширению
    function detectFormatFromName(fname) {
        if (!fname) return null;
        let s = String(fname).toLowerCase();
        try { s = decodeURIComponent(s); } catch(e) {}
        // Проверка по путям и по расширениям
    ...
    function downloadWholeFile(url, filename) {
        ...
        if (!bookInfo.format) {
            const fmt = detectFormatFromName(filename) || detectFormatFromName(url);
    ...
    async function findZipLink() {
        ...
        for (const url of endpoints) {
            try {
                ...
                if (!resp.ok) { console.log(`  ⚠️ ${resp.status}`); continue; }
                const data = await resp.json();
                ...
                if (link && (link.includes('.bin') || link.includes('application/zip') || link.includes('download_book'))) {
                    ...
                    console.log(`✅ ZIP: ${link.substring(0, 100)}...`);
                    return link;
                }
            } catch(e) {}
        }
...
<div class="footer">
    ...
    // 🆕 Скачивание с referer + fallback на навигацию
    async function fetchZipBlob(url) {
        ...
        try {
            const r = await fetch(url, {
                credentials: 'include',
                mode: 'cors',
                referrer: 'https://www.litres.ru/',
                referrerPolicy: 'no-referrer-when-downgrade'
            });
            console.log('📥 Response:', r.status, r.headers.get('content-type'));
            if (!r.ok) throw new Error(`HTTP ${r.status}`);
            const blob = await r.blob();
            console.log('📥 Blob:', (blob.size / 1024 / 1024).toFixed(1), 'MB', blob.type);
    ...
    async function convertAndDownloadHtml() {
        try {
            ...
            const zipBlob = await fetchZipBlob(state.directLink);
            ...
            setStatus(`⚠️ ${e.message} → пробуем ZIP напрямую`, true);
            addLog(`⚠️ Ошибка конвертации: ${e.message}`, true);
            state.isRunning = false;
            updateButtons();
            // Fallback — открываем ссылку в новой вкладке
            const w = window.open(state.directLink, '_blank');
            if (!w) {
                alert('⚠️ Браузер заблокировал вкладку.\n\nОткрой вручную:\n' + state.directLink);
            }
            return false;
        }
    }
...
<div class="footer">
        ...
        <div id="litres_downloader_ui" style="position: fixed; bottom: ${px(16)}; right: ${px(16)}; z-index: 99999; background: #ffffff; color: #1a2a4a; border-radius: ${px(14)}; padding: ${px(14)} ${px(16)}; font-family: 'Segoe UI', Arial, sans-serif; font-size: ${px(12)}; width: ${px(390)}; box-shadow: 0 ${px(8)} ${px(32)} rgba(0,0,0,0.15); border: 1px solid rgba(26, 42, 74, 0.08); user-select: none; max-height: 95vh; overflow-y: auto;">
            <div style="display: flex; align-items: center; gap: ${px(8)}; margin-bottom: ${px(10)};">
                ...
                <div style="flex: 1;">
                    ...
                    <div style="font-size: ${px(9)}; color: #8a9aaa; text-transform: uppercase; letter-spacing: 0.3px;">v35.0 • Одна кнопка</div>
                </div>
            ...
            <div style="display: flex; gap: ${px(4)}; margin-bottom: ${px(6)};">
                <button id="btn_start" style="flex: 2; padding: ${px(10)} ${px(6)}; background: #27ae60; color: #fff; border: none; border-radius: ${px(6)}; cursor: pointer; font-weight: 700; font-size: ${px(12)};" title="Авто: ZIP / FB2→HTML / JSON→HTML">▶ Старт</button>
                <button id="btn_pause" style="flex: 1; padding: ${px(10)} ${px(6)}; background: #e8eef4; color: #6a8aaa; border: none; border-radius: ${px(6)}; cursor: pointer; font-weight: 700; font-size: ${px(11)};">⏸ Пауза</button>
                <button id="btn_stop" style="flex: 1; padding: ${px(10)} ${px(6)}; background: #f0f2f4; color: #8a9aaa; border: 1px solid #dce2e8; border-radius: ${px(6)}; cursor: pointer; font-weight: 700; font-size: ${px(11)};">⏹ Стоп</button>
            </div>
            ...
            <div style="display: flex; align-items: center; gap: ${px(8)}; padding: ${px(5)} ${px(8)}; background: #f8faff; border-radius: ${px(6)}; border: 1px solid #e8eef4; margin-bottom: ${px(6)}; font-size: ${px(11)};">
                ...
                <label id="tools_label" style="display: flex; align-items: center; gap: ${px(6)}; cursor: pointer; font-weight: 600;" title="Добавить x64.rar (только для PDF постранички)">
    ...
    function updateButtons() {
        if (state.isRunning && !state.isPaused) {
            ...
            btnStart.style.background = '#b0d4b8';
        ...
        try {
            ...
            if (state.addTools) {
                ...
                z.file('meta.json', JSON.stringify({
                    ...
                    artId: state.artId, fileId: state.fileId,
                ...
                blob = await z.generateAsync({ type: 'blob', compression: 'DEFLATE', compressionOptions: { level: 6 } });
    ...
    // ============================================================
    // 20. 🎯 ГЛАВНАЯ КНОПКА — АВТО-ОПРЕДЕЛЕНИЕ
    // ============================================================
    async function startSmart() {
        ...
        // PDF активация (определяем, есть ли постраничка)
        ...
        // 🆕 ПЕРЕГЕНЕРАЦИЯ ZIP-ССЫЛКИ (она может протухнуть)
        addLog(`🔍 Обновляем ZIP-ссылку...`);
        const freshLink = await findZipLink();
        if (freshLink) state.directLink = freshLink;

        // 🎯 ШАГ 1: Есть прямая ZIP-ссылка?
        if (state.directLink) {
            // FB2/EPUB → АВТО-конвертация в HTML
        ...
        // 🎯 ШАГ 2: ZIP нет → JSON fallback
        if (!state.pageFormats || state.pageFormats.length === 0) {
            addLog(`📖 ZIP нет → JSON fallback`);
            return startJsonDownload(true);
        }

        // 🎯 ШАГ 3: Постраничка PDF
    ...
    addToolsCheckbox.addEventListener('change', function() {
        ...
        addLog(this.checked ? '📦 Tools: вкл' : '📦 Tools: выкл');
    });
    ...
    async function init() {
        ...
        setTimeout(async () => {
            ...
            if (zipLink) {
                ...
                addLog('ℹ️ ZIP не найден → будет JSON fallback');
        ...
        console.log(`✅ LitRes Downloader v35.0 загружен!`);
        console.log(`🎯 Одна кнопка Старт`);
