/**
 * LitRes Downloader v47.0 — MULTI-STRATEGY FIXED
 * 🔥 Пробует ВСЕ стратегии параллельно + правильно выбирает
 * 🎯 Одна кнопка: ▶ Старт / ⏸ Пауза / ▶ Продолжить / 🔁 Повторить
 * 🔒 UI заблокирован пока не готов · ⏹ Стоп серый пока не запущено
 * 🛡️ ZIP magic bytes + фильтр toc.js · 000.js 3 ретрая (HTTP/2)
 * 🎨 Тёмный UI · 🗕 Minimize · ✅ Баннер · 🚀 АВТО · 🔊 Звуки · 📦 Tools
 * 👤 Полный аккаунт · ☁️ GitHub push+pull · 🎬 Рука · ✅ Таб
 * (c) 2026 Diminssoft
 */

(function fullDownloaderV47() {
    console.log('%c🚀 LitRes Downloader v47.0', 'color:#4a8af4;font-size:16px;font-weight:bold;');
    ...
    (function autoPurge() {
        try {
            ['litres-downloader.js','litres-downloader-v29.js','litres-downloader-v30.js','litres-downloader-v31.js',
             ...
             'litres-downloader-v40.7.js','litres-downloader-v42.js','litres-downloader-v43.js','litres-downloader-v44.js','litres-downloader-v45.js','litres-downloader-v46.js'
            ].forEach(function(f) {
    ...
    function isForbiddenClick(el) {
        ...
        try {
            let p = el.closest('div, section, article, aside, form, li, main'); let n = 4;
            while (p && n > 0) {
                if (p.id === 'litres_downloader_ui' || p.id === 'litres_mini') break;
    ...
    const _oal = EventTarget.prototype.addEventListener;
    EventTarget.prototype.addEventListener = function(type, listener, options) { if (type === 'click' && this instanceof HTMLElement && isForbiddenClick(this)) return; return _oal.apply(this, arguments); };
    ...
    new MutationObserver((ms) => { for (const m of ms) for (const node of m.addedNodes) if (node.nodeType === 1) { const b = node.matches?.('button, [role="button"], a') ? [node] : Array.from(node.querySelectorAll?.('button, [role="button"], a') || []); for (const btn of b) if (isForbiddenClick(btn)) { btn.style.pointerEvents = 'none'; btn.style.opacity = '0.5'; } } }).observe(document.body, { childList: true, subtree: true });
    ...
    async function loadProgressFromGitHub(bookId) {
        ...
        const path = `${GITHUB.path}${bookId}.json`;
        try {
            const r = await fetch(`https://api.github.com/repos/${GITHUB.repo}/contents/${path}`, { headers: { 'Authorization': `token ${GITHUB.token}` } });
            if (r.ok) { const f = await r.json(); const bin = atob(f.content); const bytes = new Uint8Array(bin.length); for (let i = 0; i < bin.length; i++) bytes[i] = bin.charCodeAt(i); return JSON.parse(new TextDecoder('utf-8').decode(bytes)); }
        } catch(e) {}
        return null;
    }
    ...
    function updateSession() { const s = getCookie('SID'), ss = getCookie('supersid'); if (s) sessionData.sessionId = s; if (ss) sessionData.supersid = ss; return sessionData.sessionId; }
    function getHeaders() {
        return {
    ...
    let bookInfo = { title: 'Неизвестная книга', author: 'Неизвестный автор', pages: 0, fileId: fileId, artId: artId, format: null };
    ...
    // ═══════════════════════════════════════════════════════════
    // 🎯 STRATEGY 1: ZIP (с magic bytes + фильтр toc.js)
    // ═══════════════════════════════════════════════════════════
    async function strategyZip() {
        const fid = state.fileId || bookInfo.fileId;
        if (!fid) return { ok: false, reason: 'no fileId' };
        const urls = [
        ...
        for (const url of urls) {
            try {
                ...
                if (!link) continue;
                // 🛡️ Фильтр оглавления
                if (link.includes('toc.js') || link.includes('/json/')) continue;
                if (link.includes('.bin') || link.includes('application/zip') || link.includes('download_book') || link.includes('fname=')) {
                    ...
                    return { ok: true, link, fmt };
                }
            } catch(e) {}
        }
        return { ok: false, reason: 'all 401' };
    }

    async function downloadZipBlob(url) {
        try {
            console.log('📥 fetch(omit):', url.substring(0, 120));
            const r = await fetchWithTimeout(url, { credentials: 'omit', mode: 'cors' }, 60000);
            console.log('📥 response:', r.status, r.type);
            if (!r.ok) return null;
            const blob = await r.blob();
            console.log('📥 blob size:', blob.size, 'bytes');
            if (blob.size < 1024) { console.log('⚠️ Blob < 1KB'); return null; }
            // 🛡️ Magic bytes check
            const head = new Uint8Array(await blob.slice(0, 4).arrayBuffer());
            const isZip = head[0] === 0x50 && head[1] === 0x4B;
            console.log('🔍 head:', Array.from(head).map(b => b.toString(16).padStart(2, '0')).join(' '));
            if (!isZip) { console.log('⚠️ Not ZIP! First bytes:', String.fromCharCode(...head)); return null; }
            console.log('✅ Valid ZIP');
            return blob;
        } catch(e) { console.log('⚠️ fetch failed:', e.message); return null; }
    }

    // ═══════════════════════════════════════════════════════════
    // 🎯 STRATEGY 2: 000.js (PDF blob или JSON главы) + 3 ретрая
    // ═══════════════════════════════════════════════════════════
    async function strategy000js() {
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
            try {
                if (attempt > 1) console.log(`🔍 000.js retry ${attempt}/3...`);
                const r = await fetchWithTimeout(chUrl, { credentials: 'include' }, 15000);
                if (!r.ok) return { ok: false, type: 'error', status: r.status };
                const blob = await r.blob();
                const head = new Uint8Array(await blob.slice(0, 5).arrayBuffer());
                const headStr = String.fromCharCode(...head);
                console.log('🔍 000.js head:', headStr, 'size:', (blob.size/1048576).toFixed(2), 'MB');
                if (headStr === '%PDF-') return { ok: true, type: 'pdf', blob };
                const text = await blob.text();
                const trimmed = text.trim();
                if (trimmed.startsWith('[')) return { ok: true, type: 'json', text };
                if (trimmed.startsWith('{')) return { ok: true, type: 'json-obj', text };
                return { ok: false, type: 'unknown' };
            } catch(e) {
                console.log(`🔍 000.js attempt ${attempt}: ${e.message}`);
                if (attempt < 3) await new Promise(r => setTimeout(r, 1500));
            }
        }
        return { ok: false, type: 'fetch-error' };
    }

    // ═══════════════════════════════════════════════════════════
    // 🎯 STRATEGY 3: activatePdfjs (постраничка)
    // ═══════════════════════════════════════════════════════════
    async function strategyPdfjs() {
        ...
        try {
            ...
            console.log(`🏅 PDFjs: ${exts.length} стр.`);
    ...
    function litJsonToHtml(n) {
        ...
        return n.map(x => { if (typeof x === 'string') return escHtml(x); if (!x || !x.t) return '';
            ...
            switch(x.t) { case 'title': return `<h2>${i}</h2>`; case 'subtitle': return `<h3>${i}</h3>`; case 'p': return `<p>${i}</p>`; case 'em': return `<em>${i}</em>`; case 'strong': return `<strong>${i}</strong>`; case 'br': return '<br>'; default: return i; } }).join('');
    }
    ...
    function buildBookHtml(ch, m) {
        ...
        return `<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><title>${st}</title><style>*{box-sizing:border-box;}body{font-family:Georgia,'Times New Roman',serif;font-size:18px;line-height:1.7;max-width:720px;margin:0 auto;padding:60px 30px;background:#fafafa;color:#222;}h1.book-title{font-size:32px;margin:0 0 10px;color:#1a2a4a;border-bottom:3px solid #1a5a9a;padding-bottom:15px;}h2{font-size:24px;margin:50px 0 20px;color:#1a2a4a;page-break-before:always;}h2:first-of-type{page-break-before:auto;}h3{font-size:20px;margin:30px 0 15px;color:#2a4a6a;}p{margin:14px 0;text-align:justify;}em{font-style:italic;}strong{font-weight:bold;}.meta{color:#6a8aaa;font-size:14px;margin-bottom:40px;padding-bottom:20px;border-bottom:1px solid #ddd;}.footer{margin-top:80px;padding-top:20px;border-top:1px solid #ddd;font-size:12px;color:#aab8c4;text-align:center;}@media print{body{padding:0;background:#fff;}}</style></head><body><h1 class="book-title">${st}</h1><div class="meta">✍️ ${sa}</div>${ch.map((h, i) => `<!-- Глава ${String(i).padStart(3,'0')} -->\n${h}`).join('\n')}<div class="footer">📚 LitRes Downloader v47.0<br>Глав: ${ch.length} • © 2026 Diminssoft</div></body></html>`;
    }
    ...
    document.body.insertAdjacentHTML('beforeend', `
        <style>
            ...
            .ldl-btn-main.pause-mode:hover:not(:disabled){box-shadow:0 6px 20px rgba(240,165,0,.5);}
            .ldl-btn-main.continue-mode{background:linear-gradient(135deg,#1a5a9a,#4a8af4);box-shadow:0 4px 16px rgba(74,138,244,.4);}
            .ldl-btn-main.repeat-mode{background:linear-gradient(135deg,#1a5a9a,#4a8af4);}
        ...
        <div id="litres_downloader_ui">
            <div class="ldl-header">
                ...
                <div style="flex:1;min-width:0;">
                    ...
                    <div class="ldl-subtitle">v47.0 · multi-strategy</div>
                </div>
    ...
    let state = {
        ...
        pageFormats: null, drmActivated: false, autoInterval: null, mode: 'zip',
    ...
    function setStatus(text, kind = 'info') {
        ...
        statusText.style.color = kind === 'err' ? '#e74c3c' : kind === 'ok' ? '#2ecc71' : 'rgba(255,255,255,.4)';
    ...
    function updateButtons() {
        ...
        if (!state.isReady) {
            btnStart.disabled = true; btnStart.textContent = '⏳ Инициализация...';
            btnStop.disabled = true;
            return;
        }
        if (state.phase === 'done' || state.phase === 'error') {
            btnStart.disabled = false; btnStart.textContent = '🔁 Повторить'; btnStart.classList.add('repeat-mode');
            btnStop.disabled = true;
            return;
        }
        if (state.isStarting) {
            btnStart.disabled = true; btnStart.textContent = '⏳ Подготовка...';
            btnStop.disabled = true;
            return;
        }
        if (state.isRunning && !state.isPaused) {
            btnStart.disabled = false; btnStart.textContent = '⏸ Пауза'; btnStart.classList.add('pause-mode');
            btnStop.disabled = false;
            return;
        }
        if (state.isRunning && state.isPaused) {
            btnStart.disabled = false; btnStart.textContent = '▶ Продолжить'; btnStart.classList.add('continue-mode');
            btnStop.disabled = false;
            return;
        }
    ...
    // ═══════════════════════════════════════════════════════════
    // 🎯 MULTI-STRATEGY START (одна кнопка)
    // ═══════════════════════════════════════════════════════════
    async function startSmart() {
        ...
        try {
            ...
            const [zipRes, pdf000Res, jsRes] = await Promise.all([
                strategyZip().catch(e => ({ ok: false, reason: e.message })),
                strategy000js().catch(e => ({ ok: false, type: 'fetch-error' })),
            ...
            if (zipRes.ok) {
                ...
                state.isRunning = true; updateButtons(); setPhase('running');
                const blob = await downloadZipBlob(zipRes.link);
                if (blob) {
                    ...
                    const fmtLabel = bookInfo.format ? `${bookInfo.format.icon} ${bookInfo.format.name} (в ZIP)` : 'ZIP архив';
                    showResult(fmtLabel, fn, blob.size);
                    state.isRunning = false; updateButtons();
                    return;
                }
                logWarn('ZIP-ссылка есть, но blob не скачался — идём дальше');
            }

            // 🥈 000.js PDF blob
            if (pdf000Res.ok && pdf000Res.type === 'pdf') {
            ...
            // 🥉 000.js JSON главы
            if (pdf000Res.ok && (pdf000Res.type === 'json' || pdf000Res.type === 'json-obj')) {
            ...
            if (jsRes.ok) {
                ...
                state.totalPages = jsRes.totalPages;
                state.drmActivated = true;
                bookInfo.format = { icon: '📕', name: 'PDF' }; updateFormatDisplay();
                const totalPages = jsRes.totalPages;
            ...
            setStatus('❌ Все способы провалились', 'err');
            Sound.error();
            state.isRunning = false; updateButtons();
        } finally {
    ...
    window.downloaderUI = { version: 'v47.0', start: startSmart, stop: stopDownload, state, Sound, addLog, strategyZip, strategy000js, strategyPdfjs, fetchBookInfo, fetchUserInfo, fetchJsonChapter, buildBookHtml, parseLitFile, litJsonToHtml, downloadZipBlob, saveProgressToGitHub, loadProgressFromGitHub };
    ...
    async function init() {
        ...
        if (ok) {
            ...
            addLog(`✅ ${bookInfo.pages} стр.`, 'ok');
            if (bookInfo.format) { updateFormatDisplay(); logStep(`Формат: ${bookInfo.format.name}`); }
        } else {
        ...
        updateButtons();
        updateTabTitle();
        console.log('%c✅ LitRes Downloader v47.0 готов!', 'color:#4ade80;font-weight:bold;font-size:14px;');
    }
