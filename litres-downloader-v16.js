/**
 * LitRes Downloader v16.0
 * БЕЗ ПОКУПКИ! Правильные страницы + прогресс во вкладке
 * (c) 2026 Diminssoft
 */

(function fullDownloaderV16() {
    console.log('🚀 LitRes Downloader v16.0 — Без покупки, только скачивание!');
    ...
    // ============================================================
    // 5. ОПРЕДЕЛЯЕМ КНИГУ И СТРАНИЦЫ (ПРАВИЛЬНО!)
    // ============================================================
    let bookInfo = {
    ...
    async function fetchBookInfo() {
        try {
            ...
            // ============================================================
            // 🔥 ПРАВИЛЬНОЕ ОПРЕДЕЛЕНИЕ СТРАНИЦ
            // ============================================================
            let pages = 0;
            let symbols = payload.symbols_count || 0;

            // 1. Сначала ищем pages из файлов
            ...
            // 2. Если нет — ищем в additional_info
            if (!pages && payload.additional_info?.current_pages_or_seconds) {
                pages = payload.additional_info.current_pages_or_seconds;
            }

            // 3. Если всё ещё нет — используем символы (примерно 2800 символов на страницу)
            if (!pages && symbols > 0) {
                pages = Math.round(symbols / 2800);
            }

            // 4. Если есть release_file_id — запрашиваем информацию о файле
            ...
            // Если страниц всё ещё нет — используем символы
            if (!pages) {
    ...
    // ============================================================
    // 6. АВТООПРЕДЕЛЕНИЕ ДОСТУПНЫХ ФАЙЛОВ
    // ============================================================
    async function getAvailableFiles() {
        try {
            const resp = await fetch(
                `https://api.litres.ru/foundation/api/arts/${artId}/files`,
                {
                    method: 'GET',
                    credentials: 'include',
                    headers: getHeaders()
            ...
            for (const file of files) {
                ...
                else if (mime.includes('epub')) format = 'epub';
                else if (mime.includes('fb2')) format = 'fb2';
                else if (mime.includes('zip')) format = 'zip';
                else if (file.filename?.includes('.pdf')) format = 'pdf';
                else if (file.filename?.includes('.epub')) format = 'epub';
                else if (file.filename?.includes('.fb2')) format = 'fb2';

                // Формируем ссылку на скачивание
                const filename = file.filename || `${artId}_${file.id}.${format}`;
                const downloadUrl = `https://www.litres.ru/download_book/${artId}/${file.id}/${filename}`;
                ...
                available.push({
                    ...
                    size: file.size || 0,
                    pages: file.pages || 0,
                    is_additional: file.is_additional || false,
    ...
    // ============================================================
    // 7. ПРОВЕРКА ДОСТУПНОСТИ ФАЙЛА (HEAD)
    // ============================================================
    ...
    // ============================================================
    // 11. ОБНОВЛЕНИЕ ЗАГОЛОВКА ВКЛАДКИ
    // ============================================================
    ...
    // ============================================================
    // 15. АВТООПРЕДЕЛЕНИЕ ФОРМАТА (JPG/GIF) с HEAD-проверкой
    // ============================================================
    ...
    async function downloadLoop() {
        ...
        // Сохраняем прогресс каждые 5 страниц
    ...
    function showAvailableFiles(files) {
        ...
        filesList.innerHTML = files.map(file => `
            ...
            ">
                <span style="
                    background: ${file.format === 'pdf' ? '#e74c3c' : file.format === 'epub' ? '#3498db' : '#2ecc71'};
                    color: white;
                    padding: 2px 8px;
                    border-radius: 4px;
                    font-weight: 700;
                ...
                ">${file.format}</span>
                <span style="flex: 1; color: #1a2a4a; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                    ${file.filename}
                </span>
                <span style="color: #8a9aaa; font-size: 10px;">
                    ${file.pages > 0 ? file.pages + ' стр.' : ''}
                    ${file.size > 0 ? (Math.round(file.size / 1024) + ' KB') : ''}
                </span>
                <a href="${file.downloadUrl}" target="_blank" style="
                    background: #1a3a6a;
                    color: white;
                    padding: 4px 10px;
                    border-radius: 6px;
    ...
    async function startDownload() {
        ...
        // Используем автоопределённые страницы
    ...
    async function init() {
        ...
        if (infoLoaded && bookInfo.pages > 0) {
            ...
            addLog('🔍 Поиск доступных файлов...');
            const filesResult = await getAvailableFiles();
            if (filesResult.success && filesResult.files.length > 0) {
                state.availableFiles = filesResult.files;
                showAvailableFiles(filesResult.files);
                addLog(`✅ Найдено ${filesResult.files.length} файлов`);
                
                // Проверяем доступность каждого файла
                for (const file of filesResult.files) {
                    const available = await checkFileAvailability(file.downloadUrl);
                    addLog(`${available ? '✅' : '❌'} ${file.format}: ${file.filename}`);
                }
            } else {
                addLog('ℹ️ Доступные файлы не найдены', true);
            }
        } else {
            // Используем данные из URL или страницы
        ...
        console.log(`✅ LitRes Downloader v16.0 загружен!`);
        ...
        console.log(`⚡ FORCE-режим: выключен (включите чекбокс для быстрой загрузки)`);
        console.log(`📦 ZIP-архивация: включена автоматически`);
        console.log(`📊 Прогресс в названии вкладки: включён`);
    }
