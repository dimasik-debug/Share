...
(function fullDownloaderV33() {
    console.log('🚀 LitRes Downloader v33.0');
    ...
    (function autoPurge() {
        try {
            ...
            const filesToPurge = [
                ...
                'gh/dimasik-debug/Share@main/litres-downloader-v32.js',
                'gh/dimasik-debug/Share@main/litres-downloader-v33.js'
            ];
    ...
    // ============================================================
    // 6. API-ОПРЕДЕЛЕНИЕ КНИГИ + ФОРМАТ
    // ============================================================
    let bookInfo = {
    ...
    async function fetchBookInfo() {
        try {
            ...
            if (bookInfo.format) console.log(`📖 Формат: ${bookInfo.format.name}`);
            return true;
        } catch(e) {
    ...
    // ============================================================
    // 7. 👤 ИНФО О ПОЛЬЗОВАТЕЛЕ (ПОЛНАЯ ВЕРСИЯ)
    // ============================================================
    ...
    // ============================================================
    // 8. 🔥 АКТИВАЦИЯ PDF.js (ПОЛНАЯ ВЕРСИЯ — с Meta)
    // ============================================================
    ...
    // ============================================================
    // 10.3 🆕 TOC (оглавление для реального прогресса)
    // ============================================================
    ...
    function parseFb2Section(node, images) {
        ...
        for (const child of node.children || []) {
            ...
            switch (tag) {
                case 'section':
                    out += parseFb2Section(child, images);
                    break;
                case 'title': {
                    const t = child.textContent.trim();
                    if (t) out += `<h2>${escHtml(t)}</h2>`;
                    break;
                }
                case 'subtitle':
                    out += `<h3>${escHtml(child.textContent)}</h3>`;
                    break;
                case 'p':
                    out += `<p>${parseFb2Inline(child, images)}</p>`;
                    break;
                case 'empty-line':
                    out += '<br>';
                    break;
                case 'image': {
                    ...
                    if (id && images[id]) {
                        out += `<div style="text-align:center;margin:20px 0;"><img src="${images[id]}" style="max-width:100%;height:auto;"></div>`;
                    }
                    break;
                }
                case 'epigraph':
                    out += `<blockquote>${parseFb2Section(child, images)}</blockquote>`;
                    break;
                case 'cite':
                    out += `<blockquote class="cite">${parseFb2Section(child, images)}</blockquote>`;
                    break;
                case 'poem':
                case 'stanza':
                    out += `<div class="poem">${parseFb2Section(child, images)}</div>`;
                    break;
                case 'v':
                    out += `<p style="text-align:center;font-style:italic;margin:4px 0;">${parseFb2Inline(child, images)}</p>`;
                    break;
                case 'annotation':
                    out += `<div class="annotation"><b>Аннотация</b>${parseFb2Section(child, images)}</div>`;
                    break;
                case 'table':
                    out += `<table>${parseFb2Section(child, images)}</table>`;
                    break;
        ...
        for (const child of node.childNodes || []) {
            if (child.nodeType === 3) {
                ...
                switch (tag) {
                    ...
                    case 'a': {
                        const href = child.getAttribute('xlink:href') || child.getAttribute('href') || '#';
                        out += `<a href="${escHtml(href)}">${parseFb2Inline(child, images)}</a>`;
                        break;
                    }
                    case 'image': {
                        const href = child.getAttribute('xlink:href') || child.getAttribute('href') || '';
                        const id = href.replace('#', '');
                        if (id && images[id]) {
                            out += `<img src="${images[id]}" style="max-width:100%;height:auto;">`;
                        }
                        break;
                    }
    ...
    function buildFb2Html(title, author, bodyHtml, authors) {
        const st = escHtml(title);
        const sa = escHtml(author);
        return `<!DOCTYPE html>
<html lang="ru">
<head>
<meta charset="UTF-8">
...
<style>
    * { box-sizing: border-box; }
    body {
        ...
        padding: 60px 30px; background: #fafafa; color: #222;
    }
    h1.book-title {
        font-size: 32px; margin: 0 0 10px; color: #1a2a4a;
        border-bottom: 3px solid #1a5a9a; padding-bottom: 15px;
    }
...
${bodyHtml}
<div class="footer">
    📚 Скачано через LitRes Downloader v33.0<br>
    ${authors.length ? 'Автор: ' + authors.map(escHtml).join(', ') + '<br>' : ''}
    © 2026 Diminssoft
</div>
</body>
</html>`;
    ...
    async function convertFb2ZipToHtml(zipBlob) {
        ...
        zip.forEach((path, file) => {
            if (!file.dir && path.toLowerCase().endsWith('.fb2')) {
                fb2File = file;
            }
        });
        if (!fb2File) throw new Error('FB2 файл не найден в ZIP');
        ...
        if (parseErr) throw new Error('Ошибка парсинга XML');
        ...
        if (bodies.length > 0) {
            bodyHtml = parseFb2Section(bodies[0], images);
        }
...
<div class="footer">
    ...
    // ============================================================
    // 12. UI (ПОЛНЫЙ — с account_block, direct_links_container, book_source)
    // ============================================================
    const uiHTML = `
        ...
        ">
            ...
            <div style="display: flex; gap: ${px(4)}; margin-bottom: ${px(6)};">
                <button id="btn_start_all" style="flex: 1; padding: ${px(8)} ${px(6)}; background: #27ae60; color: #fff; border: none; border-radius: ${px(6)}; cursor: pointer; font-weight: 700; font-size: ${px(11)};" title="Скачать ВСЁ (авто формат)">▶▶ Старт</button>
            ...
            <div style="display: flex; gap: ${px(4)}; margin-bottom: ${px(6)};">
                <button id="btn_reader" style="flex: 1; padding: ${px(8)} ${px(6)}; background: #8e44ad; color: #fff; border: none; border-radius: ${px(6)}; cursor: pointer; font-weight: 700; font-size: ${px(11)};" title="Скачать как HTML через JSON-главы (ручной запуск)">📖 Reader (JSON)</button>
            </div>
            ...
            <div style="display: flex; align-items: center; gap: ${px(8)}; padding: ${px(5)} ${px(8)}; background: #f8faff; border-radius: ${px(6)}; border: 1px solid #e8eef4; margin-bottom: ${px(6)}; font-size: ${px(11)}; flex-wrap: wrap;">
                ...
                <label id="tools_label" style="display: flex; align-items: center; gap: ${px(6)}; cursor: pointer; font-weight: 600;" title="Добавить в архив x64.rar (конвертер в PDF)">
    ...
    // 🆕 Обновление отображения формата + авто-скрытие Tools
    ...
    // ============================================================
    // 19. ОСНОВНОЙ ЦИКЛ (ZIP/PDF постраничка)
    // ============================================================
...
© 2026 Diminssoft`;
    ...
    // ============================================================
    // 19.1 ЦИКЛ СКАЧИВАНИЯ JSON-ГЛАВ (с реальным прогрессом)
    // ============================================================
...
© 2026 Diminssoft`);
    ...
    async function startDownloadAll() {
        ...
        // PDF активация (для определения страниц)
        ...
        if (state.directLink) {
            // 🆕 FB2/EPUB → АВТОМАТОМ конвертируем в HTML
    ...
    // ============================================================
    // 20.1 СТАРТ JSON СКАЧИВАНИЯ (с toc.js для реального прогресса)
    // ============================================================
    async function startJsonDownload(auto = false) {
        ...
        // Fallback: используем количество страниц из API
        if (!totalChapters && state.totalPages > 0) {
            totalChapters = state.totalPages;
            addLog(`ℹ️ TOC пустой, используем pages: ${totalChapters}`);
        }

        if (!totalChapters) {
            totalChapters = 999;
            addLog(`⚠️ Число глав неизвестно, ставим 999`);
        }
    ...
    async function startDownload() {
        ...
        if (state.directLink) {
            ...
            // 🆕 FB2/EPUB — спрашиваем: HTML или оригинал?
            if (isText && !isPdf) {
            ...
            // PDF или другое — качаем как есть
            if (!isPdf) {
                addLog(`📦 Скачиваем архив напрямую...`);
                setStatus(`📦 Скачиваем архив...`);
                downloadWholeFile(state.directLink);
        ...
        if (state.jsonFallback && !state.directLink) {
            ...
            if (useJson) return startJsonDownload(true);
        }
    ...
    async function init() {
        ...
        setTimeout(async () => {
            ...
            if (user) {
                ...
                if (user.subscription) {
                    ...
                    if (user.subscription.planName) html += `<div style="font-size: ${px(9)}; color: #8a9aaa;">⚙️ ${user.subscription.planName}</div>`;
                    if (user.subscription.autoRenew) html += `<div style="font-size: ${px(9)}; color: #27ae60;">🔄 автопродление</div>`;
                    html += `</div>`;
                    addLog(`📅 Подписка до ${till.toLocaleDateString('ru-RU')} (${daysLeft} дн.)`);
                }
                if (user.account && (user.account.display || user.account.real || user.account.bonus)) {
                    ...
                    if (user.account.real) html += `<div>💵 Реальный: ${user.account.real} ₽</div>`;
                ...
                if (user.loyalty) {
                    ...
                    if (user.loyalty.purchaseForNext) html += `<div>🎯 До след. уровня: ${user.loyalty.purchaseForNext} ₽</div>`;
                    html += `</div>`;
                }
        ...
        setTimeout(async () => {
            ...
            if (zipLink) {
                ...
                setStatus(`📦 ZIP готов — жми "▶▶ Старт"`);
            } else {
                addLog('ℹ️ ZIP не найден → JSON fallback');
                if (state.jsonFallback) {
                    setStatus(`📖 Готов (JSON fallback)`);
                } else {
                    setStatus(`ℹ️ ZIP-ссылка не найдена`);
                }
            }
            updateFormatDisplay();
        }, 3000);
        ...
        console.log(`✅ LitRes Downloader v33.0 загружен!`);
        console.log(`🧹 Автоочистка кэша: включена`);
        console.log(`📦 ZIP + 📖 FB2→HTML + JSON fallback`);
        console.log(`📚 Поддержка PDF + FB2 + EPUB + HTML`);
        console.log(`📦 App Tools: ${state.addTools ? 'включены' : 'выключены'}`);
    }
