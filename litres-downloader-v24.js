...
(function fullDownloaderV23() {
    console.log('🚀 LitRes Downloader v24.0 — DRM-активация + подписка!');
  ...
  const TOOLS_PATH = 'tools/x64.rar';
    ...
    async function activatePdfjs() {
        ...
        try {
            ...
            // 3. Парсим PFURL.pdf[xxx] = {...}
            const match = jsText.match(/=\s*(\{[\s\S]*\})\s*;?\s*$/);
            if (!match) return false;
            
            const pdfData = JSON.parse(match[1]);
    ...
    async function downloadTools() {
        ...
        for (const url of TOOLS_URLS) {
            try {
                ...
                // ✅ Используем mode: 'cors' с явным указанием
                ...
                if (!resp.ok) {
                    console.warn(`⚠️ ${resp.status}: ${url.substring(0, 50)}`);
                    continue;
                }
                
                toolsBlob = await resp.blob();
                console.log(`✅ Инструменты загружены: ${(toolsBlob.size / 1024 / 1024).toFixed(2)} MB`);
                return toolsBlob;
            } catch(e) {
        ...
        console.error('❌ Все URL инструментов провалились');
        return null;
    }
