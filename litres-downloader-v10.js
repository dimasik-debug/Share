/**
 * LitRes Downloader v10.0
 * toc.js + АНИМАЦИЯ РУКИ + ПАУЗЫ + ВСЁ РАБОТАЕТ!
 * (c) 2026 Diminssoft
 */

(function fullDownloaderV10() {
    console.log('🚀 LitRes Downloader v10.0 — Полная версия с анимацией!');
    ...
    function getReadingDelay() {
        // Базовая задержка: 3-8 секунд (читает страницу)
        const baseDelay = Math.random() * 5000 + 3000;
        
        // 15% шанс залипнуть на интересной странице (10-20 секунд)
        if (Math.random() < 0.15) {
            return baseDelay + Math.random() * 10000 + 5000;
        }
        
        // 10% шанс быстро пролистать (1-2 секунды)
        if (Math.random() < 0.1) {
    ...
    function getRandomPause() {
        // 20% шанс сделать паузу 5-15 секунд
        if (Math.random() < 0.2) {
            return Math.random() * 10000 + 5000;
        }
        // 10% шанс на долгую паузу (15-45 секунд) — отвлёкся
        if (Math.random() < 0.1) {
            return Math.random() * 30000 + 15000;
        }
        // Обычная пауза 1-4 секунды
    ...
    async function downloadLoop() {
        ...
        // 4. Пауза после чтения (читатель осмысливает)
    ...
    console.log(`✅ LitRes Downloader v10.0 загружен!`);
