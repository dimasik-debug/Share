/**
 * LitRes Downloader v37.0
 * 🧹 Автоочистка кэша jsDelivr
 * 📦 Прямое скачивание ZIP (через www.litres.ru — обход CORS)
 * 🆕 📖 FB2 → HTML парсер
 * 🆕 🎯 Детект формата (FB2/EPUB/PDF)
 * 🆕 ▶ Одна кнопка Старт (авто)
 * 📖 JSON-главы (fallback)
...
(function fullDownloaderV37() {
    console.log('🚀 LitRes Downloader v37.0');
    ...
    (function autoPurge() {
        try {
            ...
            const filesToPurge = [
                ...
                'gh/dimasik-debug/Share@main/litres-downloader-v37.js'
            ];
    ...
    // ============================================================
    // 9.1 🆕 ZIP через www.litres.ru (обход CORS)
    // ============================================================
    async function tryFetchZipSameOrigin(link) {
        const sameOrigin = link.replace('content.litres.ru', 'www.litres.ru');
        console.log('🔍 Same-origin:', sameOrigin);
        try {
            const r = await fetch(sameOrigin, { credentials: 'include' });
            console.log('🔍 Response:', r.status, r.type);
            if (!r.ok) return null;
            const blob = await r.blob();
            if (blob.size < 100) return null;
            console.log('🔍 Blob:', (blob.size/1024/1024).toFixed(1), 'MB');
            return blob;
        } catch(e) {
            console.log('⚠️ Same-origin failed:', e.message);
            return null;
        }
    }

    // ============================================================
    // 10. 🔍 ПОИСК ПРЯМОЙ ZIP-ССЫЛКИ (с детектом формата)
    // ============================================================
...
${bodyHtml}
<div class="footer">
    📚 Скачано через LitRes Downloader v37.0<br>
...
© 2026 Diminssoft`);
    ...
    async function startDownloadAll() {
        ...
        // 🎯 ШАГ 1: Прямая ZIP — пробуем через www.litres.ru (same-origin)
        if (state.directLink) {
            // 🆕 Обновляем ссылку (могла протухнуть)
            addLog(`🔍 Обновляем ZIP-ссылку...`);
            const freshLink = await findZipLink();
            if (freshLink) state.directLink = freshLink;

            addLog(`🔍 Пробуем ZIP через www.litres.ru...`);
            const blob = await tryFetchZipSameOrigin(state.directLink);

            if (blob) {
                addLog(`✅ ZIP доступен (${(blob.size/1024/1024).toFixed(1)} MB)`);
                const fmtName = bookInfo.format?.name || detectFormatFromName(state.directLink)?.name;

                // FB2/EPUB → HTML
                if (fmtName === 'FB2' || fmtName === 'EPUB') {
                    addLog(`📖 ${fmtName} → HTML`);
                    state.isRunning = true;
                    updateButtons();
                    try {
                        ...
                        setStatus(`🎉 ${safeTitle}.html (${sizeMb} MB)`);
                        zipInfo.style.display = 'block';
                        zipInfo.textContent = `✅ ${safeTitle}.html (${sizeMb} MB)`;
                        zipInfo.style.color = '#1a5a9a';
                        addLog(`✅ ${safeTitle}.html`);
                        state.isRunning = false;
                        updateButtons();
                        return;
                    } catch(e) {
                        addLog(`⚠️ Конвертация: ${e.message}`, true);
                        // Fallback — сохраняем blob
                    }
                }
    ...
    async function startDownload() {
        ...
        if (state.directLink) {
            ...
            if (isText && !isPdf) {
                const asHtml = confirm(
                    ...
                    `OK → конвертировать в HTML\n` +
                    `Отмена → скачать оригинал`
                );
                if (asHtml) {
                    ...
                    if (blob) {
                        try {
                            ...
                            addLog(`⚠️ ${e.message} → оригинал`, true);
                        }
                    }
                    downloadWholeFile(state.directLink);
                    state.isRunning = false;
            ...
            const useDirect = confirm(`📦 Скачать ZIP?`);
            if (useDirect) {
                downloadWholeFile(state.directLink);
                state.isRunning = false;
                updateButtons();
        ...
        if (state.jsonFallback && !state.directLink) {
            const useJson = confirm(`📖 ZIP-ссылка не найдена.\n\nСкачать через JSON-главы?`);
    ...
    window.downloaderUI = {
        ...
        tryFetchZipSameOrigin: tryFetchZipSameOrigin,
    ...
    async function init() {
        ...
        setTimeout(async () => {
            ...
            if (zipLink) {
                ...
                if (state.jsonFallback) setStatus(`📖 Готов (JSON fallback)`);
                else setStatus(`ℹ️ ZIP-ссылка не найдена`);
            }
            updateFormatDisplay();
        }, 3000);
        ...
        console.log(`✅ LitRes Downloader v37.0 загружен!`);
