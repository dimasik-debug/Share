/**
 * LitRes Downloader v58.0 — JSON IMAGES + INFINITE RETRY + CORS DETECT
 ...
 * 🖼️ JSON IMAGES: качаем картинки из глав JSON-книг → в ZIP ✅ v58
...
(function fullDownloaderV58() {
    console.log('%c🚀 LitRes Downloader v58.0', 'color:#4a8af4;font-size:16px;font-weight:bold;');
    ...
    // v58.0 (2026-09-14): добавлена обработка узлов {t:"img"} с полем "s"
    //   Раньше картинки терялись — теперь рендерятся с реальным именем файла
    // ============================================================
    function litJsonToHtml(n){
        ...
        return n.map(x=>{
            ...
            // 🖼️ v58.0: картинки LitRes — реальное имя файла в поле "s"
            // Структура: {t:"img", src:"img4", s:"image2_...jpeg", w:591, h:532}
            if(x.t === 'img'){
                const file = x.s || x.src || '';
                if(!file) return '';
                const w = x.w ? ` width="${x.w}"` : '';
                const h = x.h ? ` height="${x.h}"` : '';
                console.log(`🖼️ litJsonToHtml: img → ${file} (${x.w||'?'}×${x.h||'?'})`);
    ...
    // ============================================================
    // КОНЕЦ litJsonToHtml v58.0 (2026-09-14)
    // ============================================================
    ...
    // v58.0 (2026-09-14): добавлен CSS для .litres-img (картинки),
    //   улучшено отображение заголовков и отступов
    // ============================================================
...
<div class="footer">📚 LitRes Downloader v58.0<br>Глав: ${ch.length} · Картинок: ${totalImgs}</div>
</body></html>`;
    }
    // ============================================================
    // КОНЕЦ buildBookHtml v58.0 (2026-09-14)
    // ============================================================

    // ============================================================
    // extractImageNames — извлекает все имена картинок из HTML-главы
    // v58.0 (2026-09-14): новая функция. Ищет <img src="...">
    //   Возвращает массив уникальных имён файлов (без пути)
    // ============================================================
    function extractImageNames(html){
        const names = new Set();
        const re = /<img[^>]+src\s*=\s*["']([^"']+\.(?:jpe?g|png|gif|webp|svg))["']/gi;
        let m;
        while((m = re.exec(html)) !== null){
            const name = m[1].replace(/^\.\//, '').replace(/^.*\//, '');
            if(name && /^image/i.test(name)) names.add(name);
        }
        return [...names];
    }
    // ============================================================
    // КОНЕЦ extractImageNames v58.0 (2026-09-14)
    // ============================================================

    // ============================================================
    // fetchBookImage — скачивает одну картинку книги
    // v58.0 (2026-09-14): новая функция. URL = baseurl книги + имя файла
    //   Возвращает { name, blob } или null при ошибке
    // ============================================================
    async function fetchBookImage(imgName){
        const cleanName = imgName.replace(/^\.\//, '').replace(/^.*\//, '');
        if(!state.fileId){
            console.log(`🖼️ fetchBookImage: нет fileId, пропускаем ${cleanName}`);
            return null;
        }
        const url = `https://www.litres.ru/download_book_subscr/${state.artId}/${state.fileId}/json/${cleanName}`;
        try{
            const r = await fetchWithTimeout(url, { credentials: 'include' }, 20000);
            if(!r.ok){
                console.log(`🖼️ fetchBookImage: ${cleanName} → HTTP ${r.status}`);
                return null;
            }
            const blob = await r.blob();
            if(blob.size < 100){
    ...
    // ============================================================
    // КОНЕЦ fetchBookImage v58.0 (2026-09-14)
    // ============================================================

    // ============================================================
    // downloadAllBookImages — параллельно качает все картинки из глав
    // v58.0 (2026-09-14): новая функция. Проходит по всем главам,
    //   собирает имена, качает по 4 параллельно, обновляет прогресс
    //   Возвращает Map<name, blob>
    // ============================================================
    async function downloadAllBookImages(chapters){
        ...
        addLog(`🖼️ Найдено картинок: ${allNames.size} — начинаем качку`, 'net');
        setReadingStatus(`🖼️ Картинки: 0/${allNames.size}`);
        animateHand('wait');
    ...
    // ============================================================
    // КОНЕЦ downloadAllBookImages v58.0 (2026-09-14)
    // ============================================================
...
<div class="footer">📚 LitRes Downloader v58.0<br>Скачано: ${new Date().toLocaleString('ru-RU')}</div>
</body></html>`;
    }
        ...
        <div id="litres_downloader_ui">
            <div class="ldl-header">
                ...
                <div style="flex:1;min-width:0;">
                    ...
                    <div class="ldl-subtitle">v58.0 · json images + retry + cors fix</div>
                </div>
    ...
    async function jsonDownloadLoop(){
        ...
        if(res.status === 'notfound'){
            ...
            // 🖼️ v58.0: логируем картинки в главе
    ...
    // v58.0 (2026-09-14): теперь качает и упаковывает ВСЕ картинки
    //   из глав книги + обложку + about.html + book_info.txt
    // ============================================================
    async function finalizeJsonBook(){
        ...
        // 🖼️ v58.0: качаем все картинки из глав
        ...
        // 🖼️ v58.0: кладём все картинки рядом с HTML
        let imgCount = 0;
    ...
    // ============================================================
    // КОНЕЦ finalizeJsonBook v58.0 (2026-09-14)
    // ============================================================
    ...
    window.downloaderUI = {
        version: 'v58.0',
        start: startSmart, stop: stopDownload, state, Sound, addLog,
    ...
    async function init(){
        ...
        updateButtons();
        updateTabTitle();
        console.log('%c✅ LitRes Downloader v58.0 готов!', 'color:#4ade80;font-weight:bold;font-size:14px;');
    }
