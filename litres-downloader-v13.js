/**
 * LitRes Downloader v13.0
 * Автоопределение JPG/GIF + FORCE-режим (без пауз)
 * (c) 2026 Diminssoft
 */

(function fullDownloaderV13() {
    console.log('🚀 LitRes Downloader v13.0 — FORCE-режим без пауз!');
    ...
    function getReadingDelay() {
        if (state.forceMode) {
            return Math.random() * 300 + 200; // Быстро: 200-500мс
        }
    ...
    function getRandomPause() {
        if (state.forceMode) {
            return Math.random() * 200 + 100; // Быстро: 100-300мс
        }
        if (Math.random() < 0.2) {
    ...
    // ============================================================
    // 12. АВТООПРЕДЕЛЕНИЕ ФОРМАТА (JPG/GIF) с HEAD-проверкой
    // ============================================================
    ...
    // ============================================================
    // 13. СКАЧИВАНИЕ СТРАНИЦЫ
    // ============================================================
    function downloadViaImg(imageUrl, pageNum, ext) {
        return new Promise((resolve) => {
            ...
            img.onload = function() {
                ...
                try {
                    ...
                    canvas.toBlob((blob) => {
                        if (blob) {
                            const fileName = `${state.bookTitle}_page_${String(pageNum).padStart(3, '0')}.${ext}`;
                            ...
                            console.log(`✅ Стр. ${pageNum} → ${fileName}`);
                            resolve(true);
                        } else {
                            resolve(false);
                        }
    ...
    async function downloadLoop() {
        ...
        // Анимация чтения (всегда, даже в FORCE)
        ...
        // Анимация перелистывания (всегда)
        setReadingStatus(`🔄 Перелистываем стр. ${pageNum}...`);
        animateHand('turn');
        await new Promise(resolve => setTimeout(resolve, 800));

        // Получаем ссылку с автоопределением формата
        const result = await getImageUrl(pageNum);
        let success = false;
        
        if (result) {
            success = await downloadViaImg(result.url, pageNum, result.ext);
        }
        ...
        // Пауза после чтения (СОКРАЩЕНА в FORCE)
    ...
    async function startDownload() {
        ...
        // ============================================================
        // С какой страницы начать и СКОЛЬКО страниц скачать
        // ============================================================
        ...
        if (startPage < 1 || startPage > state.totalPages) {
        ...
        const endPage = Math.min(startPage + pagesCount - 1, state.totalPages);
    ...
    console.log(`✅ LitRes Downloader v13.0 загружен!`);
