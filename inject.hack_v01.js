/**
 * YM+MTS Music Interceptor v2.0 — FULL SNIFFER + IMPORTANT ENDPOINTS
 * 🕵️ Перехват: fetch, XHR, cookies, JSON, секреты
 * ⭐ Классификация: Yandex Music API + MTS Music API
 * 🔑 Автоизвлечение OAuth-токенов из cookies
 * 📊 Per-endpoint статистика · 🎯 Фильтр "важные только"
 * 💾 Экспорт TXT с разделами по сервисам
 * ⏯ Start / Pause / Stop · 🗕 Minimize · 🔊 Звуки
 * (c) 2026 Diminssoft
 */
...
(function ymMtsInterceptorV2() {
    ...
    const Sound = {
        ...
        important(){ this.chord([880,1108,1318],0.25,0.09,'triangle'); },   // ⭐ важный эндпоинт
    ...
    // ═══════════════════════════════════════════════════════════
    // ⭐ РЕЕСТР ВАЖНЫХ ЭНДПОИНТОВ (Yandex + MTS)
    // ═══════════════════════════════════════════════════════════
    // importance: 3 = 🔴 критично, 2 = 🟡 важно, 1 = 🟢 интересно
    const IMPORTANT_ENDPOINTS = [
        // ============ YANDEX MUSIC — критичные ============
        { re:/\/get-file-info\b/i,                    name:'get-file-info',       service:'yandex', importance:3, desc:'Получение ссылки на mp3 (с sign)' },
        { re:/\/download-info\b/i,                    name:'download-info',       service:'yandex', importance:3, desc:'DownloadInfoUrl (fallback)' },
        { re:/\/plays\b/i,                            name:'plays',               service:'yandex', importance:3, desc:'Логирование прослушивания' },
        { re:/log\.strm\.yandex\.ru/i,                name:'log.strm (beacon)',   service:'yandex', importance:3, desc:'Плеер-беконы: PlayerAlive, SetSource' },
        { re:/\/get-mp3\//i,                          name:'get-mp3 (CDN)',       service:'yandex', importance:3, desc:'CDN-ссылка на mp3' },
        { re:/\/account\/status\b/i,                  name:'account/status',      service:'yandex', importance:3, desc:'Статус аккаунта (uid, plus)' },
        { re:/\/account\/login/i,                     name:'account/login',       service:'yandex', importance:3, desc:'Логин/сессия' },

        // ============ YANDEX MUSIC — важно ============
        { re:/\/search\/instant\/mixed/i,             name:'search/instant',      service:'yandex', importance:2, desc:'Поиск (треки/альбомы)' },
        { re:/\/tracks\/[^\/]+$/i,                    name:'tracks/{id}',         service:'yandex', importance:2, desc:'Инфо о треке' },
        { re:/\/albums\/[^\/]+\/with-tracks/i,        name:'albums/with-tracks',  service:'yandex', importance:2, desc:'Альбом с треками' },
        { re:/\/playlist\/[^\/]+/i,                   name:'playlist/{uuid}',     service:'yandex', importance:2, desc:'Плейлист' },
        { re:/\/landing\//i,                          name:'landing/*',           service:'yandex', importance:2, desc:'Лендинги (жанры, новые релизы)' },
        { re:/\/chart\b/i,                            name:'chart',               service:'yandex', importance:2, desc:'Чарты' },
        { re:/\/artists\/[^\/]+\/track-ids/i,         name:'artist/track-ids',    service:'yandex', importance:2, desc:'Топ-треки артиста' },
        { re:/\/metatags\//i,                         name:'metatags',            service:'yandex', importance:2, desc:'Плейлисты по метатегам' },
        { re:/\/non-music\//i,                        name:'non-music',           service:'yandex', importance:2, desc:'Аудиокниги/подкасты' },
        { re:/\/queues\b/i,                           name:'queues',              service:'yandex', importance:2, desc:'Очередь воспроизведения' },
        { re:/\/likes\b/i,                            name:'likes',               service:'yandex', importance:2, desc:'Лайки/избранное' },
        { re:/\/users\/[^\/]+\/playlists/i,           name:'user/playlists',      service:'yandex', importance:2, desc:'Плейлисты пользователя' },
        { re:/\/music-history\b/i,                    name:'music-history',       service:'yandex', importance:2, desc:'История прослушиваний' },
        ...
        // ============ MTS MUSIC — критичные ============
        { re:/music\.mts\.ru\/ya-proxy/i,             name:'MTS ya-proxy',        service:'mts',    importance:3, desc:'MTS проксирует Яндекс API' },
        { re:/\/api\/account\/mts\/status/i,          name:'MTS account/status',  service:'mts',    importance:3, desc:'Статус MTS-аккаунта' },
        { re:/api\.music\.yandex\.net\/tracks\/[^\/]+\/download-info/i, name:'MTS download-info', service:'mts', importance:3, desc:'MTS: download info' },
        { re:/api\.music\.yandex\.net\/get-file-info/i, name:'MTS get-file-info',  service:'mts',    importance:3, desc:'MTS: sign + transport' },
        { re:/\/ya-proxy\/api\/get-mp3\//i,           name:'MTS get-mp3 proxy',   service:'mts',    importance:3, desc:'MTS: финальный CDN' },
        ...
        { re:/music\.mts\.ru\/api\//i,                name:'MTS /api/*',          service:'mts',    importance:1, desc:'MTS прочее API' },
    ];

    // Секреты/токены в JSON
    ...
    const state = {
        ...
        endpointStats: new Map(),  // name -> {count, service, importance, lastUrl, lastTime}
        maxLogs: 5000,
    ...
    function classifyUrl(url) {
        if (!url) return null;
        for (const ep of IMPORTANT_ENDPOINTS) {
            if (ep.re.test(url)) return ep;
        }
        return null;
    }
    ...
    function recordCookieHeader(setCookieStr, source){
        ...
        for(const part of parts){
            ...
            if(!m) continue;
            recordCookie(m[1].trim(), m[2].trim(), source);
        }
    }
    ...
    window.fetch = async function(input, init){
        ...
        if (endpoint) {
            ...
            if (/captcha/i.test(url)) {
                state.counters.captcha++;
                addLog('🚨 ОБНАРУЖЕНА КАПЧА!', 'err');
            }
        } else {
            Sound.request();
        }
        ...
        try{
            ...
            (async ()=>{
                try{
                    ...
                    else { bodyText = '[binary '+(clone.headers.get('content-length')||'?')+' bytes]'; }
                    ...
                    log({ type:'fetch_body_error', url, error:String(e) });
                }
            })();
    ...
    XMLHttpRequest.prototype.send = function(body){
        ...
        log({ type:'xhr_request', url, method, headers:this.__ym_headers||{}, body: typeof body==='string'?body:(body?'['+(body.constructor&&body.constructor.name)+']':null), endpoint: endpoint ? {name:endpoint.name, service:endpoint.service, importance:endpoint.importance} : null });

        const secrets = [];
        for(const re of SECRET_PATTERNS){
            const m = url.match(new RegExp(re.source+'=([^&]+)','i'));
            if(m) secrets.push({ path:'url', key:re.source, value:m[1].slice(0,200) });
        }
        for(const [k,v] of Object.entries(this.__ym_headers||{})){
            for(const re of SECRET_PATTERNS){
                if(re.test(k)){ secrets.push({ path:'header.'+k, key:k, value:String(v).slice(0,200) }); break; }
            }
        }
        if(secrets.length) recordSecrets(secrets, url+' [XHR-REQUEST]');
        ...
        const onDone = ()=>{
            ...
            try{
                ...
                try{
                    ...
                    else bodyText = '['+this.responseType+']';
                }catch(e){}
                ...
                state.counters.errors++;
                log({ type:'xhr_response_error', url, error:String(e) });
            }
        };
    ...
    // ═══════════════════════════════════════════════════════════
    // ПЕРЕХВАТ document.cookie (setter)
    // ═══════════════════════════════════════════════════════════
    try{
    ...
    function buildReport(){
        ...
        L.push('═'.repeat(78));
        L.push('');
        L.push('Создан: ' + now.toLocaleString('ru-RU'));
        L.push('URL страницы: ' + location.href);
        ...
        if(state.startedAt){ const end = state.stoppedAt || Date.now(); L.push('Длительность: ' + ((end-state.startedAt)/1000).toFixed(1) + ' сек'); }
        L.push('Фильтр URL: ' + (state.urlFilter || '(нет)'));
        L.push('Только важные: ' + (state.onlyImportant ? 'ДА' : 'нет'));
        L.push('');
        ...
        L.push('  📋 Логов:      ' + state.logs.length);
        L.push('');
        ...
        if (state.endpointStats.size === 0) {
            L.push('  (важные эндпоинты не вызывались)');
        } else {
            // Группируем по сервису
            const byService = { yandex: [], mts: [], other: [] };
            for (const [name, st] of state.endpointStats.entries()) {
                const grp = byService[st.service] || byService.other;
                grp.push({ name, ...st });
            }
            for (const svc of ['yandex', 'mts', 'other']) {
                const items = byService[svc];
                if (!items.length) continue;
                const svcName = svc === 'yandex' ? '🎵 YANDEX MUSIC' : svc === 'mts' ? '📱 MTS MUSIC' : '❓ OTHER';
                L.push('');
                L.push(`  ${svcName}:`);
                items.sort((a,b)=> b.count - a.count);
                for (const it of items) {
                    const impStars = '⭐'.repeat(it.importance);
                    L.push(`    ${impStars} ${it.name.padEnd(28)} × ${String(it.count).padStart(4)}  — ${it.desc || ''}`);
                }
            }
        }
        L.push('');
        ...
        L.push('');
        L.push('═'.repeat(78));
        L.push('  🔑 НАЙДЕННЫЕ СЕКРЕТЫ (' + state.secrets.length + ')');
        L.push('═'.repeat(78));
        if(!state.secrets.length) L.push('  (не найдено)');
        else state.secrets.forEach((s,i)=>{
            L.push('');
            L.push(`[${i+1}] ${s.t}`);
            L.push(`    Источник: ${s.source}`);
            L.push(`    Путь:     ${s.path}`);
            L.push(`    Ключ:     ${s.key}`);
            L.push(`    Значение: ${s.value}`);
        });
        L.push('');

        // 🍪 КУКИ (с пометкой токенов)
        L.push('');
        L.push('═'.repeat(78));
        L.push('  🍪 КУКИ (' + state.cookies.size + ')');
        L.push('═'.repeat(78));
        if(!state.cookies.size) L.push('  (нет)');
        else{
            const arr = Array.from(state.cookies.entries()).sort((a,b)=>{
                const aTok = a[1].isToken ? 0 : 1;
                const bTok = b[1].isToken ? 0 : 1;
                if (aTok !== bTok) return aTok - bTok;
                return a[0].localeCompare(b[0]);
            });
            for(const [name, info] of arr){
                const tokenMark = info.isToken ? ' 🔑' : '';
                L.push('');
                L.push(`  Name:      ${name}${tokenMark}`);
                L.push(`  Value:     ${info.value}`);
                L.push(`  First:     ${info.firstSeen}`);
                L.push(`  Last:      ${info.lastSeen}`);
                L.push(`  Source:    ${info.source}`);
                if(info.changes) L.push(`  Changes:   ${info.changes}`);
            }
        }
        L.push('');
        ...
        L.push('');
        L.push('═'.repeat(78));
        L.push('  📜 ПОЛНЫЙ ЛОГ (' + state.logs.length + ' записей)');
        L.push('═'.repeat(78));
        L.push('');
        state.logs.forEach((e,i)=>{
            L.push('');
            L.push('─'.repeat(78));
            const epMark = e.endpoint ? ` [⭐ ${e.endpoint.service.toUpperCase()}:${e.endpoint.name}]` : '';
            ...
            switch(e.type){
                case 'fetch_request':
                case 'xhr_request':
                    L.push(`  ${e.method} ${e.url}`);
                    if(e.headers && Object.keys(e.headers).length){
                        L.push('  ── Заголовки запроса:');
                        for(const [k,v] of Object.entries(e.headers)) L.push(`     ${k}: ${String(v).slice(0,500)}`);
                    }
                    if(e.body) { L.push('  ── Тело запроса:'); L.push(indent(String(e.body).slice(0,10000))); }
                    break;
                case 'fetch_response':
                case 'xhr_response':
                    L.push(`  ← ${e.status} ${e.method} ${e.url}`);
                    if(e.elapsed_ms!=null) L.push(`  Время: ${e.elapsed_ms} мс`);
                    if(e.headers && Object.keys(e.headers).length){
                        L.push('  ── Заголовки ответа:');
                        for(const [k,v] of Object.entries(e.headers)) L.push(`     ${k}: ${String(v).slice(0,500)}`);
                    }
                    if(e.body) { L.push('  ── Тело ответа:'); L.push(indent(String(e.body).slice(0,30000))); }
                    break;
                case 'fetch_error':
                case 'xhr_error':
                    L.push(`  ❌ ${e.method} ${e.url}`);
                    L.push(`  Ошибка: ${e.error}`);
                    break;
                case 'fetch_body_error':
                case 'xhr_response_error':
                    L.push(`  ⚠️ ${e.url}`);
                    L.push(`  Ошибка тела: ${e.error}`);
                    break;
                case 'secret_found':
                    L.push(`  🔑 ${e.url}`);
                    L.push(`  Найдено: ${e.count}`);
                    (e.secrets||[]).forEach(s => L.push(`    • ${s.path} = ${String(s.value).slice(0,300)}`));
                    break;
                case 'cookie_new':
                    L.push(`  🍪 Новая: ${e.name}${e.isToken ? ' 🔑' : ''}`);
                    L.push(`  Значение: ${e.value}`);
                    L.push(`  Источник: ${e.source}`);
                    break;
                case 'cookie_change':
                    L.push(`  🍪 Изменение: ${e.name}`);
                    L.push(`  Новое: ${e.value}`);
                    break;
                case 'system':
                    L.push(`  ℹ️ ${e.message||''}`);
                    break;
                default:
                    L.push('  ' + safeStringify(e));
            }
        });
        L.push('');
        L.push('═'.repeat(78));
    ...
    function downloadTxt(){
        ...
        log({ type:'system', message:'Отчёт скачан ('+report.length+' символов)' });
        Sound.save();
    }
    ...
    document.body.insertAdjacentHTML('beforeend', `
        <style>
            ...
            @keyframes ym-flash{0%,100%{background:rgba(255,204,0,.08);}50%{background:rgba(255,204,0,.25);}}
    ...
    function renderEndpointStats(){
        ...
        for (const it of items) {
            ...
            html += `<div class="ym-endpoint-row ${cls}">
                <span class="name">${svcIcon} ${escapeHtml(it.name)}</span>
    ...
    function updateUI(){
        ...
        if(state.running && !state.paused){
            ...
            logo.classList.add('rec');
            statusMain.textContent = '● ИДЁТ ЗАПИСЬ';
            statusMain.style.color = '#ff0055';
            ...
            statusMain.textContent = '⏸ Пауза';
            statusMain.style.color = '#f0a500';
            statusPhase.textContent = 'Нажми СТАРТ для продолжения';
        } else if(state.startedAt){
            pulse.classList.remove('rec'); logo.classList.remove('rec');
            statusMain.textContent = '⏹ Остановлен';
            statusMain.style.color = '#666';
            statusPhase.textContent = 'Готов к экспорту';
        } else {
            pulse.classList.remove('rec'); logo.classList.remove('rec');
            statusMain.textContent = '⏸ Ожидание';
            statusMain.style.color = '#888';
    ...
    function updateMini(){
        ...
        if(state.running && !state.paused){
            ...
            miniBadge.textContent = 'REC';
            miniBadge.style.background = '#ff0055';
            miniLogo.classList.add('rec');
        } else if(state.paused){
            miniStatus.textContent = '⏸ пауза';
    ...
    function updateButtons(){
        ...
        if(!state.running){
            ...
            btnSave.disabled = !state.logs.length;
            return;
        }
        if(state.paused){
            btnStart.textContent = '▶ ПРОДОЛЖИТЬ';
            btnStart.classList.add('continue-mode');
        } else {
            btnStart.textContent = '⏸ ПАУЗА';
            btnStart.classList.add('pause-mode');
        }
        btnStart.disabled = false;
    ...
    btnStart.addEventListener('click', ()=>{
        if(!state.running){
            ...
            addLog(`▶ Перехват запущен. Найдено ${tokenCount} токенов в куках`, 'ok');
    ...
    btnStop.addEventListener('click', ()=>{
        ...
        addLog(`⏹ Остановлен. Запросов: ${state.counters.requests}, важных: ${state.counters.important}, секретов: ${state.counters.secrets}`, 'warn');
    ...
    onlyImportantCb.addEventListener('change', function(){
        ...
        addLog(this.checked ? '⭐ Только важные эндпоинты' : '🌐 Все запросы', 'step');
        Sound.click();
    });
    ...
    window.YM_MTS_INTERCEPTOR = {
        ...
        getEndpointStats: ()=>{
            return Object.fromEntries(state.endpointStats.entries());
        },
        getCookies: ()=>{
            return Object.fromEntries(state.cookies.entries());
        },
        getCookiesRaw: ()=>document.cookie,
        getTokenCookies: ()=>{
            const result = {};
            for (const [name, info] of state.cookies.entries()) {
                if (info.isToken) result[name] = info.value;
            }
            return result;
        },
        ...
        // Получить последний ответ от эндпоинта
        lastResponseOf: (endpointName)=>{
            for (let i = state.logs.length - 1; i >= 0; i--) {
                const l = state.logs[i];
                if (l.endpoint && l.endpoint.name === endpointName && (l.type === 'fetch_response' || l.type === 'xhr_response')) {
                    return l;
                }
            }
            return null;
    ...
    console.log(
        ...
        '%c⭐ Отслеживает ' + IMPORTANT_ENDPOINTS.length + ' важных эндпоинтов (Yandex + MTS)\n' +
    ...
    updateUI();
    updateButtons();
    addLog(`⭐ ${IMPORTANT_ENDPOINTS.length} важных эндпоинтов в реестре. Нажми СТАРТ`, 'ok');
