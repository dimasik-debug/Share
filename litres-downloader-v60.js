/**
 * LitRes Downloader v60.0 — STRATEGY DIAGNOSTICS + SAVINGS + JSON IMAGES
 * 🔬 DIAGNOSTICS: сначала проверяем ВСЕ стратегии и выводим таблицу ✅ v60
 * 🎯 PRIORITY: PDF → ZIP → Audio/Video → JSON→HTML → JPG/GIF постранично
 * 📦 resource=toc.js → прямая ссылка на ZIP книги ✅ v60
 ...
 * 🖼️ JSON IMAGES: качаем картинки из глав JSON-книг → в ZIP
 * 💰 SAVINGS: счётчик сэкономленных рублей + история книг
 * 🔍 Универсальный детект через magic bytes
 * 📊 Прогресс-бар · 🚀 Автостарт · 🗕 Minimize · 🔊 Звуки
 * (c) 2026 Diminssoft
 */

(function fullDownloaderV60() {
    console.log('%c🚀 LitRes Downloader v60.0', 'color:#4a8af4;font-size:16px;font-weight:bold;');
    ...
    // ============================================================
    // SAVINGS — учёт сэкономленных денег
    // v60.0 (2026-09-14): объект для хранения суммы экономии
    // ============================================================
    const SAVINGS = {
        total: 0,
        currency: 'RUB',
        ...
        load(){
            try{
                ...
                if(raw){
                    ...
                    this.books = data.books || 0;
                    this.history = Array.isArray(data.history) ? data.history : [];
                    console.log(`💰 SAVINGS: загружено ${this.total.toFixed(2)} ${this.currency} (${this.books} книг)`);
                }
            }catch(e){ console.warn('⚠️ SAVINGS load:', e.message); }
        },
        save(){
            try{
                localStorage.setItem(SAVINGS_KEY, JSON.stringify({
                    total: this.total, currency: this.currency, books: this.books,
                    history: this.history.slice(-100)
                }));
            }catch(e){ console.warn('⚠️ SAVINGS save:', e.message); }
        },
        add(artId, title, price){
            if(!price || price <= 0){ console.log(`💰 SAVINGS: цена не определена для "${title}" — пропускаем`); return; }
            this.total += price;
            this.books += 1;
        ...
        reset(){
            ...
            this.save(); updateSavingsDisplay();
            addLog('💰 Счётчик экономии сброшен', 'warn');
        }
    };
    // ============================================================
    // КОНЕЦ SAVINGS v60.0 (2026-09-14)
    // ============================================================
    ...
    // v60.0 (2026-09-14): + price, images_count, isDraft
    // ============================================================
    async function fetchBookInfo(){
        try{
            ...
            // 🎯 v60: определяем черновик/фрагмент
            bookInfo.isDraft = !!(p.is_draft || p.art_type === 1 || (p.my_art_status === 0 && !p.files?.length));
            console.log(`✅ "${bookInfo.title}" (${bookInfo.pages} ${bookInfo.isAudio?'сек':'стр.'}) · ${bookInfo.reviewsCount} рец. · ${bookInfo.imagesCount} 🖼️ · ${bookInfo.price ? bookInfo.price+'₽' : 'бесплатно'}${bookInfo.isDraft?' · 📝 draft':''}`);
            return true;
        }catch(e){ console.error('❌',e); return false; }
    }
    // ============================================================
    // КОНЕЦ fetchBookInfo v60.0 (2026-09-14)
    // ============================================================
    ...
    // ============================================================
    // formatPrice — красивое форматирование цены
    // v60.0 (2026-09-14): 239.9 → "239,90 ₽"
    // ============================================================
    ...
    // ============================================================
    // КОНЕЦ formatPrice v60.0 (2026-09-14)
    // ============================================================
    ...
    // ═══════════════════════════════════════════════════════════
    // 📊 downloadWithProgress — INFINITE RETRY ×100 + ADAPTIVE DELAY + RANGE-RESUME
    // ═══════════════════════════════════════════════════════════
        ...
        while(attempt < MAX_ATTEMPTS){
            ...
            try{
                ...
                if(attempt >= MAX_ATTEMPTS){
                    addLog(`❌ ${label}: ${MAX_ATTEMPTS} попыток исчерпано (${fmtBytes(loaded)})`, 'err');
    ...
    // ============================================================
    // ═══════════════════════════════════════════════════════════
    // 🔬 v60.0: ДИАГНОСТИКА СТРАТЕГИЙ
    //   Проверяет все возможные способы скачивания ПАРАЛЛЕЛЬНО
    ...
    // ============================================================
    // checkStrategy_Pdf — прямой PDF через API link
    // v60.0 (2026-09-14): проверяем ?is_trial=false, ищем .pdf в fname
    // ============================================================
    async function checkStrategy_Pdf(fid){
        const r = { ok:false, link:null, note:'', name:'📕 PDF (прямой)' };
        try{
            const url = `https://api.litres.ru/foundation/api/arts/files/${fid}/link?is_trial=false`;
            const resp = await fetchWithTimeout(url, {credentials:'include', headers:getHeaders()}, 8000);
            ...
            if(/\.pdf$/i.test(fname) || /fname=[^&]*\.pdf/i.test(link)){
                r.ok = true; r.link = link;
                r.note = fname || 'PDF';
            } else {
                r.note = `link есть, но не PDF (fname="${fname||'?'}")`;
            }
        }catch(e){ r.note = e.message; }
        return r;
    }
    // ============================================================
    // КОНЕЦ checkStrategy_Pdf v60.0 (2026-09-14)
    // ============================================================

    // ============================================================
    // checkStrategy_ZipToc — ZIP через resource=toc.js
    // v60.0 (2026-09-14): ГЛАВНАЯ НАХОДКА — отдаёт ZIP даже для драфтов!
    // ============================================================
    async function checkStrategy_ZipToc(fid){
        const r = { ok:false, link:null, note:'', name:'📦 ZIP (toc.js)' };
        try{
            const url = `https://api.litres.ru/foundation/api/arts/files/${fid}/link?resource=toc.js&is_trial=false`;
            const resp = await fetchWithTimeout(url, {credentials:'include', headers:getHeaders()}, 8000);
            if(!resp.ok){ r.note = `HTTP ${resp.status}`; return r; }
            const d = await resp.json();
            const link = d?.payload?.data?.link || d?.payload?.link || '';
            ...
            const m = link.match(/fname=([^&]+)/);
            const fname = m ? decodeURIComponent(m[1]) : '';
            if(/\.zip$/i.test(fname) || /\.zip(\?|$)/i.test(link) || /download_book.*fname=.*\.zip/i.test(link)){
                r.ok = true; r.link = link;
                r.note = fname || 'ZIP';
            } else if(link.includes('toc.js')){
                r.note = `link есть, но fname="${fname||'?'}" — не ZIP`;
            } else {
                r.note = `неожиданный link (fname="${fname||'?'}")`;
            }
        }catch(e){ r.note = e.message; }
        return r;
    }
    // ============================================================
    // КОНЕЦ checkStrategy_ZipToc v60.0 (2026-09-14)
    // ============================================================

    // ============================================================
    // checkStrategy_ZipDirect — ZIP напрямую через ?is_trial=false
    // v60.0 (2026-09-14): для случаев когда toc.js нет, но ZIP есть
    // ============================================================
    async function checkStrategy_ZipDirect(fid){
        ...
        try{
            ...
            for(const url of urls){
                ...
                const m = link.match(/fname=([^&]+)/);
                const fname = m ? decodeURIComponent(m[1]) : '';
                if(/\.zip$/i.test(fname) || /\.zip(\?|$)/i.test(link) || /\.bin(\?|$)/i.test(link)){
                    r.ok = true; r.link = link;
                    r.note = fname || 'ZIP/bin';
                    return r;
                }
            }
            r.note = 'не найдено';
    ...
    // ============================================================
    // КОНЕЦ checkStrategy_ZipDirect v60.0 (2026-09-14)
    // ============================================================

    // ============================================================
    // checkStrategy_Audio — аудио/видео (mp3, m4b, m4a, mp4 и т.д.)
    // v60.0 (2026-09-14): проверяем resource=mp3/m4b/m4a/mp4
    // ============================================================
    async function checkStrategy_Audio(fid){
        ...
        try{
            const urls = [
                ...
                `https://api.litres.ru/foundation/api/arts/files/${fid}/link?resource=m4a&is_trial=false`,
                `https://api.litres.ru/foundation/api/arts/files/${fid}/link?resource=mp4&is_trial=false`
            ];
            for(const url of urls){
                const resp = await fetchWithTimeout(url, {credentials:'include', headers:getHeaders()}, 6000);
                if(!resp.ok) continue;
    ...
    // ============================================================
    // КОНЕЦ checkStrategy_Audio v60.0 (2026-09-14)
    // ============================================================

    // ============================================================
    // checkStrategy_Pdfjs — PDFjs манифест (постраничная сборка JPG/GIF)
    // v60.0 (2026-09-14): index=1 → pdfjs.js → массив exts
    // ============================================================
    async function checkStrategy_Pdfjs(fid){
        const r = { ok:false, link:null, note:'', name:'📕 PDFjs (постранично)', pages:0, pageFormats:null };
        try{
            const url = `https://api.litres.ru/foundation/api/arts/files/${fid}/link?index=1&is_trial=false`;
            const resp = await fetchWithTimeout(url, {credentials:'include', headers:getHeaders()}, 8000);
            if(!resp.ok){ r.note = `HTTP ${resp.status}`; return r; }
            const d = await resp.json();
            const jsUrl = d?.payload?.data?.link || d?.payload?.link || '';
            if(!jsUrl){ r.note = 'нет link на pdfjs'; return r; }
            ...
            if(exts.length === 0){ r.note = 'нет ext в pdfjs'; return r; }
            r.ok = true; r.link = jsUrl;
            r.pages = exts.length;
            r.pageFormats = exts;
            r.note = `${exts.length} стр. (${[...new Set(exts)].join(',')})`;
        }catch(e){ r.note = e.message; }
        return r;
    }
    // ============================================================
    // КОНЕЦ checkStrategy_Pdfjs v60.0 (2026-09-14)
    // ============================================================

    // ============================================================
    // checkStrategy_Json — JSON-главы (000.js) Range-проверка
    // v60.0 (2026-09-14): проверяем первые байты 000.js
    // ============================================================
    ...
    // ============================================================
    // КОНЕЦ checkStrategy_Json v60.0 (2026-09-14)
    // ============================================================

    // ============================================================
    // diagnoseStrategies — ГЛАВНАЯ функция диагностики
    // v60.0 (2026-09-14): запускает все проверки параллельно,
    //   рисует таблицу в консоли, возвращает результаты
    // ============================================================
    async function diagnoseStrategies(){
        ...
        console.log('%c🔬 ДИАГНОСТИКА СТРАТЕГИЙ v60.0', 'color:#4a8af4;font-size:14px;font-weight:bold;');
        console.log('%c═══════════════════════════════════════════════════════', 'color:#4a8af4');
        ...
        // ─── Обновляем UI ───
        updateDiagnosticsDisplay(results);

        // ─── Обновляем формат книги по приоритету ───
        if(pdf.ok && !bookInfo.format){
            bookInfo.format = { icon:'📕', name:'PDF' }; updateFormatDisplay();
        } else if(zipToc.ok && !bookInfo.format){
            bookInfo.format = { icon:'📦', name:'ZIP' }; updateFormatDisplay();
        } else if(audio.ok && !bookInfo.format){
            bookInfo.format = audio.format; updateFormatDisplay();
        }

        return results;
    }
    // ============================================================
    // КОНЕЦ diagnoseStrategies v60.0 (2026-09-14)
    // ============================================================

    // ============================================================
    // updateDiagnosticsDisplay — отрисовка диагностики в UI
    // v60.0 (2026-09-14): новая функция. Показывает список
    //   стратегий и их статус в маленьком блоке под карточкой книги
    // ============================================================
    function updateDiagnosticsDisplay(results){
        try{
            const block = document.getElementById('diag_block');
            const text = document.getElementById('diag_text');
            ...
            const rows = [];
            const order = ['pdf', 'zipToc', 'zipDirect', 'audio', 'pdfjs', 'json'];
            const icons = {
                pdf: '📕', zipToc: '📦', zipDirect: '📦', audio: '🎵', pdfjs: '📕', json: '📖'
            };
            const labels = {
                pdf: 'PDF прямой', zipToc: 'ZIP toc.js', zipDirect: 'ZIP direct',
                audio: 'Audio/Video', pdfjs: 'PDFjs постр.', json: 'JSON главы'
            };
            for(const key of order){
                const r = results[key];
                if(!r) continue;
                const icon = icons[key] || '•';
                const label = labels[key] || r.name;
                const mark = r.ok ? '<b style="color:#2ecc71;">✅</b>' : '<b style="color:#5a6a7a;">❌</b>';
                const note = r.note ? `<span style="color:rgba(255,255,255,.4);font-size:10px;"> — ${r.note}</span>` : '';
                rows.push(`<div>${mark} ${icon} ${label}${note}</div>`);
            }
            text.innerHTML = rows.join('');
        }catch(e){ console.warn('updateDiagnosticsDisplay:', e.message); }
    }
    // ============================================================
    // КОНЕЦ updateDiagnosticsDisplay v60.0 (2026-09-14)
    // ============================================================
    ...
    // ============================================================
    // litJsonToHtml — конвертер JSON-глав LitRes в HTML
    // v60.0 (2026-09-14): картинки — поле "s", fallback на "src"
    // ============================================================
    ...
    // ============================================================
    // КОНЕЦ litJsonToHtml v60.0 (2026-09-14)
    // ============================================================
    ...
    // ============================================================
    // buildBookHtml — собирает единый HTML из глав
    // v60.0 (2026-09-14): CSS для .litres-img, счётчик картинок
    // ============================================================
...
<div class="footer">📚 LitRes Downloader v60.0<br>Глав: ${ch.length} · Картинок: ${totalImgs}</div>
</body></html>`;
    }
    // ============================================================
    // КОНЕЦ buildBookHtml v60.0 (2026-09-14)
    // ============================================================

    // ============================================================
    // extractImageNames — имена картинок из HTML-главы
    // v60.0 (2026-09-14): без фильтра /^image/, ловим "_0.jpg" тоже
    // ============================================================
    ...
    // ============================================================
    // КОНЕЦ extractImageNames v60.0 (2026-09-14)
    // ============================================================

    // ============================================================
    // fetchBookImage — скачивает одну картинку
    // v60.0 (2026-09-14): 3 fallback-пути + проверка magic bytes
    // ============================================================
    async function fetchBookImage(imgName){
        ...
        const urls = [`${base}/${cleanName}`, `${base}/images/${cleanName}`, `${base}/img/${cleanName}`];
        for(const url of urls){
            try{
    ...
    // ============================================================
    // КОНЕЦ fetchBookImage v60.0 (2026-09-14)
    // ============================================================

    // ============================================================
    // downloadAllBookImages — параллельно качает все картинки
    // v60.0 (2026-09-14): по 4 параллельно, прогресс в UI
    // ============================================================
    async function downloadAllBookImages(chapters){
        ...
        const more = list.length > 3 ? ` … ещё ${list.length - 3}` : '';
        addLog(`🖼️ Найдено: ${list.length} [${sample}${more}]`, 'net');
        ...
        for(let i = 0; i < list.length; i += CONCURRENCY){
            ...
            blobs.forEach((b, idx) => {
                if(b){ result.set(b.name, b.blob); done++; }
                else { failed++; logWarn(`🖼️ Не скачалась: ${batch[idx]}`); }
            });
            const pct = Math.round((done + failed) / list.length * 100);
            setReadingStatus(`🖼️ ${done}/${list.length} (${pct}%)`);
            readingProgressText.textContent = `🖼️ ${done} ok · ${failed} fail`;
            updateMini();
        }
        addLog(`✅ Картинок: ${done}/${list.length}${failed?` (ошибок: ${failed})`:''}`, done>0?'ok':'warn');
        return result;
    }
    // ============================================================
    // КОНЕЦ downloadAllBookImages v60.0 (2026-09-14)
    // ============================================================

    // ═══════════════════════════════════════════════════════════
    // 🖼️ ОБЛОЖКА + РЕЦЕНЗИИ + ABOUT.HTML
    // ═══════════════════════════════════════════════════════════
    ...
    function buildAboutHtml(book, reviews){
        ...
        return `<!DOCTYPE html>
<html lang="ru"><head><meta charset="UTF-8"><title>${st} — О книге</title>
<style>
*{box-sizing:border-box;}
...
${revHtml}
<div class="footer">📚 LitRes Downloader v60.0<br>Скачано: ${new Date().toLocaleString('ru-RU')}</div>
</body></html>`;
    }
    ...
    async function packMetaIntoZip(zip, artId, book){
        ...
        try{
            ...
            if(cov){ zip.file(`cover.${cov.ext}`, cov.blob); packed.cover = cov.ext; }
            const reviews = await fetchReviews(artId, 20);
            zip.file('about.html', buildAboutHtml(book, reviews));
            packed.about = true;
    ...
    document.body.insertAdjacentHTML('beforeend', `
        <style>
            ...
            #litres_downloader_ui{animation:ldl-in .35s cubic-bezier(.16,1,.3,1);position:fixed;bottom:16px;right:16px;z-index:99999;background:#0e0e10;color:#fff;font-family:'Segoe UI',Arial,sans-serif;width:420px;max-width:calc(100vw - 32px);border-radius:20px;border:1px solid rgba(255,255,255,.08);box-shadow:0 24px 80px rgba(0,0,0,.8);overflow:hidden;display:flex;flex-direction:column;max-height:calc(100vh - 32px);user-select:none;font-size:13px;}
            ...
            .ldl-card.diag-card{background:rgba(0,0,0,.3);border-left:3px solid #7c5cff;font-family:'SF Mono',Consolas,monospace;font-size:10px;}
        ...
        <div id="litres_downloader_ui">
            <div class="ldl-header">
                ...
                <div style="flex:1;min-width:0;">
                    ...
                    <div class="ldl-subtitle">v60.0 · strategy diagnostics + savings</div>
                </div>
            ...
            <div class="ldl-body">
                ...
                <div class="ldl-card diag-card" id="diag_block" style="display:none;">
                    <div class="ldl-card-label">🔬 Диагностика стратегий</div>
                    <div id="diag_text" style="line-height:1.7;"></div>
                </div>
    ...
    function addLog(text, kind='info'){
        logStatus.textContent = `${({ok:'✓',err:'✕',warn:'⚠',step:'▸',net:'🌐',db:'💾',info:'ℹ️',money:'💰'})[kind]||'ℹ️'} [${new Date().toLocaleTimeString()}] ${text}`;
    ...
    // ============================================================
    // updateSavingsDisplay — обновляет UI счётчика экономии
    // v60.0 (2026-09-14): красивое форматирование суммы
    // ============================================================
    function updateSavingsDisplay(){
        ...
        }catch(e){ console.warn('⚠️ updateSavingsDisplay:', e.message); }
    }
    // ============================================================
    // КОНЕЦ updateSavingsDisplay v60.0 (2026-09-14)
    // ============================================================

    // ============================================================
    // pulseSavings — анимация пульсации счётчика
    // v60.0 (2026-09-14)
    // ============================================================
    function pulseSavings(){
        try{
            if(!savingsBlock) return;
            savingsBlock.classList.remove('pulse');
            void savingsBlock.offsetWidth;
            savingsBlock.classList.add('pulse');
            setTimeout(()=>savingsBlock.classList.remove('pulse'), 1300);
        }catch(e){}
    }
    // ============================================================
    // КОНЕЦ pulseSavings v60.0 (2026-09-14)
    // ============================================================
    ...
    // ============================================================
    // showResult — баннер успеха + начисление экономии
    // v60.0 (2026-09-14): добавлена экономия
    // ============================================================
    ...
    // ============================================================
    // КОНЕЦ showResult v60.0 (2026-09-14)
    // ============================================================
    ...
    // ═══ Скачивание страниц для PDF-постранички (JPG/GIF) ═══
    ...
    async function jsonDownloadLoop(){
        ...
        if(res.status === 'notfound'){
            ...
            if(state.jsonNotFoundStreak >= 5){ logStep(`🏁 Конец книги — глав: ${state.jsonChapters.length}`); await finalizeJsonBook(); return; }
    ...
    // ============================================================
    // finalizeJsonBook — JSON-книга: HTML + картинки → ZIP
    // v60.0 (2026-09-14): качаем все картинки из глав, кладём рядом
    // ============================================================
    async function finalizeJsonBook(){
        ...
        state.zip.file('book_info.txt',
            ...
            `Картинок: ${imgCount}${bookInfo.imagesCount ? ' (в API: '+bookInfo.imagesCount+')' : ''}\n` +
            `Цена: ${bookInfo.price ? formatPrice(bookInfo.price) : '—'}\n` +
            `Формат: HTML (из JSON)\nОбложка: ${packed.cover ? 'cover.'+packed.cover : 'нет'}\n` +
            `Дата: ${new Date().toLocaleString('ru-RU')}\nСкачано через LitRes Downloader v60.0`
        );
    ...
    // ============================================================
    // КОНЕЦ finalizeJsonBook v60.0 (2026-09-14)
    // ============================================================
    ...
    // ============================================================
    // startSmart — главный запуск
    // v60.0 (2026-09-14): СНАЧАЛА ДИАГНОСТИКА ВСЕХ СТРАТЕГИЙ,
    //   потом выбор по приоритету:
    //   PDF → ZIP → Audio/Video → JSON → JPG/GIF
    // ============================================================
    async function startSmart(){
        ...
        try{
            ...
            // ─── 🔬 ЭТАП 1: ДИАГНОСТИКА ───
            logStep('🔬 Этап 1: Диагностика всех стратегий...');
            const diag = await diagnoseStrategies();
            state.diagnostics = diag;
            if(!diag){
                setStatus('❌ Диагностика провалилась', 'err');
                Sound.error();
                return;
            }

            // ─── 🎯 ЭТАП 2: ВЫБОР ПО ПРИОРИТЕТУ ───
            // Приоритет: PDF → ZIP(toc.js) → ZIP(direct) → Audio/Video → JSON → JPG/GIF
            ...
            if(diag.pdf.ok){
                ...
                logWarn('PDF не скачался → пробуем следующую стратегию');
            }

            // 2️⃣ ZIP через toc.js
            if(diag.zipToc.ok){
                logStep('📦 ZIP через toc.js (приоритет 2)');
                state.isRunning = true; updateButtons();
                bookInfo.format = { icon:'📦', name:'ZIP' }; updateFormatDisplay();
                const blob = await downloadWithProgress(diag.zipToc.link, { credentials:'omit', mode:'cors' }, 'ZIP', ['zip']);
                if(blob && blob.size > 1024){
                    const safe = state.bookTitle.replace(/[\\/:*?"<>|]/g,'_').slice(0,100);
                    const fn = `${safe}.zip`;
                    triggerDownload(blob, fn);
                ...
                logWarn('ZIP (toc.js) не скачался → следующая стратегия');
            }
            ...
            if(diag.zipDirect.ok){
                ...
                state.isRunning = true; updateButtons();
                bookInfo.format = { icon:'📦', name:'ZIP' }; updateFormatDisplay();
                const blob = await downloadWithProgress(diag.zipDirect.link, { credentials:'omit', mode:'cors' }, 'ZIP', ['zip']);
                if(blob && blob.size > 1024){
                    const safe = state.bookTitle.replace(/[\\/:*?"<>|]/g,'_').slice(0,100);
                    const fn = `${safe}.zip`;
                    triggerDownload(blob, fn);
                ...
                logWarn('ZIP direct не скачался → следующая стратегия');
            }
            ...
            if(diag.audio.ok){
                ...
                const blob = await downloadWithProgress(diag.audio.link, { credentials:'omit', mode:'cors' }, diag.audio.format.name, expected);
                if(blob && blob.size > 1024){
                ...
                logWarn('Мультимедиа не скачалось → следующая стратегия');
            }

            // 5️⃣ JSON главы → HTML + картинки
            if(diag.json.ok){
            ...
            if(diag.pdfjs.ok){
                logStep(`📕 JPG/GIF постранично — ${diag.pdfjs.pages} стр. (приоритет 6)`);
                state.pageFormats = diag.pdfjs.pageFormats;
                state.totalPages = diag.pdfjs.pages;
            ...
            Sound.error();
            logErr('Все стратегии недоступны');
        } finally {
    ...
    // ============================================================
    // КОНЕЦ startSmart v60.0 (2026-09-14)
    // ============================================================
    ...
    window.downloaderUI = {
        ...
        diagnoseStrategies, updateDiagnosticsDisplay,
    ...
    async function init(){
        ...
        updateButtons();
        updateTabTitle();
        console.log('%c✅ LitRes Downloader v60.0 готов!', 'color:#4ade80;font-weight:bold;font-size:14px;');
    ...
    async function handleUrlChange(){
        ...
        try{
            ...
            updateButtons();
            const ok = await fetchBookInfo();
            if(ok){ previewBookTitle.textContent = bookInfo.title; previewBookAuthor.textContent = bookInfo.author; previewTotalPages.textContent = bookInfo.pages; updatePriceDisplay(); state.bookTitle = bookInfo.title; state.bookAuthor = bookInfo.author; state.totalPages = bookInfo.pages; state.fileId = bookInfo.fileId; state.bookInfoLoaded = true; updateFormatDisplay(); }
            state.isReady = true;
