/**
 * LitRes Downloader v18.0
 * API определение книги + Рабочий toc.js + Все фичи
 * (c) 2026 Diminssoft
 */

(function fullDownloaderV18() {
    console.log('🚀 LitRes Downloader v18.0 — API + toc.js + Все фичи!');
    ...
    // ============================================================
    // 5. ОПРЕДЕЛЯЕМ КНИГУ ЧЕРЕЗ API (ПРАВИЛЬНО!)
    // ============================================================
    let bookInfo = {
    ...
    // ============================================================
    // 6. ПОИСК toc.js (РАБОТАЕТ!)
    // ============================================================
    async function findTocLink() {
        try {
            const resp = await fetch(
                `https://api.litres.ru/foundation/api/arts/files/${fileId}/link?resource=toc.js&is_trial=false`,
                {
                    method: 'GET',
                    credentials: 'include',
                    headers: getHeaders()
    ...
    // ============================================================
    // 10. ОБНОВЛЕНИЕ ЗАГОЛОВКА ВКЛАДКИ
    // ============================================================
    ...
    function showDirectLink(url) {
        ...
        addLog(`✅ Найдена прямая ссылка: ${url}`);
    }

    // ============================================================
    // 17. АВТООПРЕДЕЛЕНИЕ ФОРМАТА (JPG/GIF) с HEAD-проверкой
    // ============================================================
    ...
    async function startDownload() {
        ...
        // Если есть прямая ссылка — показываем
        if (state.directLink) {
            ...
            if (useDirect) {
                ...
                addLog(`📎 Открыта ссылка: ${state.directLink}`);
                ...
                if (copyChoice) {
                    try {
                        ...
                        addLog('📋 Ссылка скопирована в буфер обмена');
                    } catch(e) {
    ...
    async function init() {
        ...
        console.log(`✅ LitRes Downloader v18.0 загружен!`);
        ...
        console.log(`🔍 Поиск toc.js: запущен`);
        console.log(`📡 API определение книги: включено`);
    }
