/**
 * LitRes Downloader v59.0 — SAVINGS COUNTER + JSON IMAGES + INFINITE RETRY
 ...
 * 🖼️ JSON IMAGES: качаем картинки из глав JSON-книг → в ZIP ✅ v58.1
 *   · без фильтра /^image/ — ловим имена вида "_0.jpg" и "image0_xxx.jpeg"
 *   · fallback-пути: /json/, /json/images/, /json/img/
 *   · fallback поля: s → src
 * 💰 SAVINGS: счётчик сэкономленных рублей + история книг ✅ v59
...
(function fullDownloaderV59() {
    console.log('%c🚀 LitRes Downloader v59.0', 'color:#4a8af4;font-size:16px;font-weight:bold;');
    ...
    ['litres-downloader.js','litres-downloader-v29.js','litres-downloader-v30.js','litres-downloader-v31.js',
     ...
     'litres-downloader-v57.js','litres-downloader-v58.js','litres-downloader-v58.1.js'
    ...
    // v59.0 (2026-09-14): объект для хранения суммы экономии.
    //   Загружается из localStorage при старте, сохраняется при
    //   каждом успешном скачивании. Хранит историю последних 100 книг.
    // ============================================================
    const SAVINGS = {
        total: 0,
        currency: 'RUB',
        ...
        add(artId, title, price){
            if(!price || price <= 0){
                console.log(`💰 SAVINGS: цена не определена для "${title}" — пропускаем`);
                return;
            }
            this.total += price;
            this.books += 1;
    ...
    // ============================================================
    // КОНЕЦ SAVINGS v59.0 (2026-09-14)
    // ============================================================
    ...
    // v59.0 (2026-09-14): добавлено извлечение цены (prices.final_price)
    //   и images_count для отображения экономии и ожидаемых картинок.
    // ============================================================
    async function fetchBookInfo(){
        try{
            ...
            console.log(`✅ "${bookInfo.title}" (${bookInfo.pages} ${bookInfo.isAudio?'сек':'стр.'}) · ${bookInfo.reviewsCount} рец. · ${bookInfo.imagesCount} 🖼️ · ${bookInfo.price ? bookInfo.price+'₽' : 'бесплатно'}`);
            return true;
        }catch(e){ console.error('❌',e); return false; }
    }
    // ============================================================
    // КОНЕЦ fetchBookInfo v59.0 (2026-09-14)
    // ============================================================
    ...
    // v59.0 (2026-09-14): 239.9 → "239,90 ₽", 1234.5 → "1 234,50 ₽"
    // ============================================================
    ...
    // ============================================================
    // КОНЕЦ formatPrice v59.0 (2026-09-14)
    // ============================================================
    ...
    // v59.0 (2026-09-14): картинки LitRes. Основное поле "s".
    //   Если "s" нет — fallback на "src", но только если оно похоже
    //   на файл с расширением картинки.
    // ============================================================
    function litJsonToHtml(n){
        ...
        return n.map(x=>{
            ...
            if(x.t === 'img'){
                let file = x.s || '';
                if(!file && x.src && /\.(?:jpe?g|png|gif|webp|svg|bmp)$/i.test(x.src)){
                    file = x.src;
                }
                if(!file){
                    console.log(`🖼️ litJsonToHtml: img без имени (src="${x.src||''}")`);
                    return '';
                }
    ...
    // ============================================================
    // КОНЕЦ litJsonToHtml v59.0 (2026-09-14)
    // ============================================================
    ...
    // v59.0 (2026-09-14): CSS для .litres-img, счётчик картинок в футере
    // ============================================================
...
<div class="footer">📚 LitRes Downloader v59.0<br>Глав: ${ch.length} · Картинок: ${totalImgs}</div>
</body></html>`;
    }
    // ============================================================
    // КОНЕЦ buildBookHtml v59.0 (2026-09-14)
    // ============================================================

    // ============================================================
    // extractImageNames — извлекает имена картинок из HTML-главы
    // v59.0 (2026-09-14): без фильтра /^image/ — ловим и "_0.jpg",
    //   и "image0_xxx.jpeg". Принимаем любое имя с картинным расширением.
    // ============================================================
    ...
    // ============================================================
    // КОНЕЦ extractImageNames v59.0 (2026-09-14)
    // ============================================================

    // ============================================================
    // fetchBookImage — скачивает одну картинку книги
    // v59.0 (2026-09-14): три fallback-пути (/json/, /json/images/,
    //   /json/img/). Проверка magic bytes — что это реально картинка.
    // ============================================================
    async function fetchBookImage(imgName){
        const cleanName = imgName.replace(/^\.\//, '').replace(/^.*\//, '').trim();
        if(!state.fileId){
            console.log(`🖼️ fetchBookImage: нет fileId, пропускаем ${cleanName}`);
            return null;
        }
        ...
        for(const url of urls){
            try{
                ...
                if(!r.ok){
                    console.log(`🖼️ fetchBookImage: ${cleanName} → ${url.replace(base,'…')} → HTTP ${r.status}`);
                    continue;
                }
                const blob = await r.blob();
                if(blob.size < 100){
                    console.log(`🖼️ fetchBookImage: ${cleanName} слишком мал (${blob.size} B)`);
                    continue;
                }
                ...
                if(!isJpg && !isPng && !isGif && !isWebp){
                    console.log(`🖼️ fetchBookImage: ${cleanName} — не картинка (magic: ${Array.from(head).map(b=>b.toString(16)).join(' ')})`);
                    continue;
                }
                console.log(`✅ fetchBookImage: ${cleanName} ← ${(blob.size/1024).toFixed(1)} KB`);
                return { name: cleanName, blob };
            }catch(e){
                console.log(`🖼️ fetchBookImage: ${cleanName} → ${e.message}`);
            }
        }
        return null;
    }
    // ============================================================
    // КОНЕЦ fetchBookImage v59.0 (2026-09-14)
    // ============================================================

    // ============================================================
    // downloadAllBookImages — параллельно качает все картинки
    // v59.0 (2026-09-14): по 4 параллельно, прогресс в UI,
    //   показывает первые 3 имени для отладки
    // ============================================================
    async function downloadAllBookImages(chapters){
        ...
        if(allNames.size === 0){
            addLog('🖼️ Картинок в книге не найдено', 'info');
            return new Map();
        }
        ...
        addLog(`🖼️ Найдено картинок: ${list.length} [${sample}${more}]`, 'net');
        setReadingStatus(`🖼️ Картинки: 0/${list.length}`);
        animateHand('wait');
        ...
        for(let i = 0; i < list.length; i += CONCURRENCY){
            ...
            setReadingStatus(`🖼️ Картинки: ${done}/${list.length} (${pct}%)`);
            readingProgressText.textContent = `🖼️ ${done} ok · ${failed} fail из ${list.length}`;
            updateMini();
        }

        addLog(`✅ Картинок скачано: ${done}/${list.length}${failed?` (ошибок: ${failed})`:''}`, done>0?'ok':'warn');
        return result;
    }
    // ============================================================
    // КОНЕЦ downloadAllBookImages v59.0 (2026-09-14)
    // ============================================================
...
<div class="footer">📚 LitRes Downloader v59.0<br>Скачано: ${new Date().toLocaleString('ru-RU')}</div>
</body></html>`;
    }
        ...
        <div id="litres_downloader_ui">
            <div class="ldl-header">
                ...
                <div style="flex:1;min-width:0;">
                    ...
                    <div class="ldl-subtitle">v59.0 · savings + json images</div>
                </div>
    ...
    // ============================================================
    // updateSavingsDisplay — обновление UI счётчика экономии
    // v59.0 (2026-09-14): новая функция. Красиво форматирует сумму
    //   через formatPrice и обновляет мета-строку (количество книг).
    // ============================================================
    ...
    // ============================================================
    // КОНЕЦ updateSavingsDisplay v59.0 (2026-09-14)
    // ============================================================

    // ============================================================
    // pulseSavings — краткая анимация пульсации счётчика
    // v59.0 (2026-09-14): новая функция. Мигает рамкой при пополнении.
    // ============================================================
    ...
    // ============================================================
    // КОНЕЦ pulseSavings v59.0 (2026-09-14)
    // ============================================================
    ...
    function updatePriceDisplay(){
        ...
        if(bookInfo.price && bookInfo.price > 0){
            el.innerHTML = `💰 <b style="color:#2ecc71;">${formatPrice(bookInfo.price)}</b>`;
        }else if(bookInfo.price === 0){
            el.innerHTML = `🆓 <b style="color:#2ecc71;">бесплатно</b>`;
        }else{
            el.innerHTML = '';
        }
    }
    ...
    // ============================================================
    // showResult — показывает баннер успеха + добавляет экономию
    // v59.0 (2026-09-14): добавлено начисление в SAVINGS.add() и
    //   отображение сэкономленной суммы в баннере + пульсация счётчика.
    // ============================================================
    function showResult(format, filename, size){
        ...
        // 💰 v59.0: начисляем экономию один раз на книгу
        let moneyLine = '';
        if(!state.savingsApplied && bookInfo.price && bookInfo.price > 0){
            ...
            moneyLine = `<div class="ldl-result-line">🆓 Книга бесплатная — экономия не начисляется</div>`;
            state.savingsApplied = true;
        }
    ...
    // ============================================================
    // КОНЕЦ showResult v59.0 (2026-09-14)
    // ============================================================
    ...
    // ============================================================
    // finalizeJsonBook — финализация JSON-книги: HTML + картинки → ZIP
    // v59.0 (2026-09-14): качает ВСЕ картинки из глав, кладёт рядом
    //   с HTML, добавляет в book_info.txt цену и счётчик картинок.
    // ============================================================
    async function finalizeJsonBook(){
        ...
        try{
            imagesMap = await downloadAllBookImages(state.jsonChapters);
        }catch(e){
            logWarn(`🖼️ Ошибка качки картинок: ${e.message}`);
        }
        ...
        try{
            const zb = await state.zip.generateAsync({
                type: 'blob',
                compression: 'DEFLATE',
    ...
    // ============================================================
    // КОНЕЦ finalizeJsonBook v59.0 (2026-09-14)
    // ============================================================
    ...
    // 💰 v59.0: кнопка сброса счётчика экономии
    if(btnSavingsReset){
    ...
    window.downloaderUI = {
        ...
        strategyZip, strategy000js, strategyPdfjs, downloadWithProgress, detectBlobType,
    ...
    async function init(){
        // 💰 v59.0: загружаем счётчик экономии
        SAVINGS.load();
        updateSavingsDisplay();
        ...
        updateButtons();
        updateTabTitle();
        console.log('%c✅ LitRes Downloader v59.0 готов!', 'color:#4ade80;font-weight:bold;font-size:14px;');
    ...
    async function handleUrlChange(){
        ...
        try{
            ...
            if(state.autoInterval){ clearTimeout(state.autoInterval); state.autoInterval=null; }
            state.isReady = false;
            Object.assign(state, { isRunning:false, isPaused:false, isStopped:true, downloaded:0, total:0, totalPages:0, pageFormats:null, drmActivated:false, fileId:null, artId:newArtId, directLink:null, bookInfoLoaded:false, jsonChapters:[], mode:'zip', phase:'loading', savingsApplied:false });
