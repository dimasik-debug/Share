/**
 * LitRes Downloader v7.0
 * Анимация руки + реалистичные паузы + окошко со ссылкой
 * (c) 2026 Diminssoft
 */

(function fullDownloaderV7() {
    console.log('🚀 LitRes Downloader v7.0 — С окошком ссылки');
    ...
    // ============================================================
    // 6. ПОИСК ПРЯМОЙ ССЫЛКИ
    // ============================================================
    async function tryDirectDownload() {
        try {
            const resp = await fetch(
                `https://api.litres.ru/foundation/api/arts/files/${fileId}/link?resource=full&is_trial=false`,
                {
                    method: 'GET',
                    credentials: 'include',
                    headers: getHeaders()
            ...
            if (url && (url.includes('.zip') || url.includes('.fb2') || url.includes('.epub'))) {
                return url;
            }
            return null;
        } catch(e) {
    ...
    async function downloadLoop() {
        ...
        // Пауза после перелистывания (читатель осмысливает)
    ...
    async function startDownload() {
        ...
        const directUrl = await tryDirectDownload();
        if (directUrl) {
            // ПОКАЗЫВАЕМ ОКОШКО С ССЫЛКОЙ!
            const userChoice = confirm(
                `📚 Найдена прямая ссылка на книгу!\n\n` +
                `📖 ${bookTitle}\n` +
                `📦 Формат: ZIP-архив\n\n` +
                `Ссылка:\n${directUrl}\n\n` +
                `✅ Нажмите "OK" — откроется ссылка в новой вкладке\n` +
            ...
            if (userChoice) {
                ...
                addLog(`📎 Открыта ссылка: ${directUrl}`);
                
                // Показываем ссылку в логе
                ...
                if (copyChoice) {
                    try {
                        ...
                        // Если не получилось скопировать, показываем окошко с текстом
                        prompt('📋 Скопируйте ссылку вручную:', directUrl);
                    }
                }
    ...
    console.log(`✅ LitRes Downloader v7.0 загружен!`);
