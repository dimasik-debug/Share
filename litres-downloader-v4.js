/**
 * LitRes Downloader v4.0
 * С сохранением прогресса на GitHub
 * (c) 2026 Diminssoft
 */

(function fullDownloaderV4() {
    console.log('🚀 LitRes Downloader v4.0 — С сохранением на GitHub');
    ...
    const GITHUB_CONFIG = {
        ...
        // Папка для хранения прогресса
        path: 'books/progress/',
        // Токен запрашиваем у пользователя при первом запуске
    ...
    async function saveProgressToGitHub(bookId, data) {
        ...
        try {
            // Сначала получаем SHA (если файл существует)
            let sha = '';
            try {
    ...
    // Получение списка всех сохранённых книг
    async function listSavedBooks() {
        ...
        try {
            ...
            console.warn('⚠️ Не удалось получить список книг:', e);
        }
        return [];
    }
        ...
        ">
            ...
            ">
                ...
                <div style="font-size: 12px; color: #4a6a8a; margin-top: 4px; border-top: 1px solid #e8eef4; padding-top: 4px;">
                    ☁️ Прогресс: <span id="github_status" style="color: #1a5a9a;">не сохранён</span>
                </div>
            </div>
    ...
    function updateFileNameDisplay() {
        if (state.bookTitle && state.startPage && state.endPage) {
            ...
            fileNameDisplay.textContent = fileName + '.zip';
            document.getElementById('preview_range').textContent = `${state.startPage} - ${state.endPage}`;
            document.getElementById('preview_filename').textContent = fileName + '.zip';
        }
    }
    ...
    async function saveProgress() {
        ...
        const now = Date.now();
        if (now - state.lastSaveTime < 30000) return; // не чаще 30 секунд
    ...
    async function checkForSavedProgress() {
        ...
        if (progress && progress.downloaded_pages > 0 && progress.downloaded_pages < progress.total_pages) {
            ...
            if (resume) {
                ...
                // Загружаем уже скачанные страницы (если есть)
    ...
    function isBookFinished() {
        ...
        // Проверка реального конца книги (если пользователь ввёл больше страниц, чем есть)
    ...
    async function startDownload() {
        ...
        if (!sessionData.sessionId) {
            showError('⚠️ Не найден session-id. Обновите страницу!');
    ...
    console.log(`✅ LitRes Downloader v4.0 загружен!`);
