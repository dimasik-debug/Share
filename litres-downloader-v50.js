/**
 * LitRes Downloader v50.0 — MULTIMEDIA
 ...
 * ⚡ Range-проверка 000.js — быстро
 * 📊 Прогресс-бар · 🚀 Автостарт · 🗕 Minimize · 🔊 Звуки
 * (c) 2026 Diminssoft
 */

(function fullDownloaderV50() {
    console.log('%c🚀 LitRes Downloader v50.0 MULTIMEDIA', 'color:#4a8af4;font-size:16px;font-weight:bold;');
    ...
    ['litres-downloader.js','litres-downloader-v29.js','litres-downloader-v30.js','litres-downloader-v31.js',
     ...
     'litres-downloader-v45.js','litres-downloader-v46.js','litres-downloader-v47.js','litres-downloader-v48.js','litres-downloader-v49.js'
    ...
    // ═══════════════════════════════════════════════════════════
    // 🔍 УНИВЕРСАЛЬНЫЙ ДЕТЕКТ ТИПА КОНТЕНТА (magic bytes)
    // ═══════════════════════════════════════════════════════════
    async function detectBlobType(blob){
        ...
        // 🎵 MP3 frame sync: FF Fx (xxxxx=E0 mask)
        ...
        // 🎧 MP4/M4B/M4A/MP4-video: ....ftyp на 4-8
        if(str(4,4)==='ftyp'){
        ...
        if(head[0]===0x1A && head[1]===0x45 && head[2]===0xDF && head[3]===0xA3){
            return { type:'video-webm', icon:'🎬', name:'WEBM', mime:'video/webm', hex };
        }
    ...
    function isBookZipFormat(name){ return ['FB2','EPUB','TXT','MOBI','ZIP'].includes(name); }

    // ═══════════════════════════════════════════════════════════
    // 📊 DOWNLOAD WITH PROGRESS + AUTO-DETECT
    // ═══════════════════════════════════════════════════════════
    async function downloadWithProgress(url, opts={}, label='файла', expectedTypes=null){
        ...
        try{
            ...
            // 🛡️ Auto-detect
            const info = await detectBlobType(blob);
            console.log(`🔍 ${label} определён: ${info.icon||''} ${info.name} (${fmtBytes(blob.size)})`, info.hex ? `| head: ${info.hex}` : '');
            if(expectedTypes && !expectedTypes.includes(info.type)){
                console.log(`⚠️ ${label}: тип "${info.type}" не в списке ожидаемых`);
    ...
    // ═══════════════════════════════════════════════════════════
    // 🎯 STRATEGY 1: ZIP / MULTIMEDIA (по ссылке)
    // ═══════════════════════════════════════════════════════════
    async function strategyZip(){
        ...
        for(const url of urls){
            try{
                ...
                if(link.includes('/pdfjs/')) continue;
                if(link.includes('mimetype=text/javascript')) continue;
                if(link.includes('fname=') && link.includes('.pdf')) continue;
                const isReal = link.includes('.bin') || link.includes('.zip') || link.includes('application/zip') ||
                              ...
                              (link.includes('download_book') && !link.includes('/pdfjs/'));
                if(!isReal) continue;
                ...
                console.log('🥇 Direct link:', link.substring(0, 120));
                return { ok:true, link };
            }catch(e){}
        }
        return { ok:false };
    ...
    // ═══════════════════════════════════════════════════════════
    // 🎯 STRATEGY 2: 000.js с RANGE header (быстрая проверка!)
    // ═══════════════════════════════════════════════════════════
    async function strategy000js(){
        ...
        for(let attempt=1; attempt<=3; attempt++){
            try{
                if(attempt>1) addLog(`⏳ 000.js retry ${attempt}/3...`, 'warn');
                // ⚡ Быстрая проверка через Range header — только первые 16 байт
                let headInfo = null;
                let needFull = true;
                try {
                    const rHead = await fetchWithTimeout(chUrl, { credentials:'include', headers:{ 'Range':'bytes=0-15' } }, 6000);
                    if(rHead.ok || rHead.status===206){
                        const buf = await rHead.arrayBuffer();
                        const head = new Uint8Array(buf);
                        const str = (s,l) => String.fromCharCode(...head.slice(s,s+l));
                        if(str(0,5)==='%PDF-') headInfo = { type:'pdf' };
                        else if(str(0,3)==='ID3') headInfo = { type:'audio-mp3' };
                        else if(str(0,4)==='fLaC') headInfo = { type:'audio-flac' };
                        else if(str(0,4)==='OggS') headInfo = { type:'audio-ogg' };
                        else if(str(4,4)==='ftyp') headInfo = { type:'audio-m4a' };
                        else if(head[0]===0x1A && head[1]===0x45 && head[2]===0xDF && head[3]===0xA3) headInfo = { type:'video-webm' };
                        else if(str(0,1)==='[') headInfo = { type:'json' };
                        else if(str(0,1)==='{') headInfo = { type:'json-obj' };
                        else if(head[0]===0xFF && (head[1]&0xE0)===0xE0) headInfo = { type:'audio-mp3' };
                        console.log(`🔍 000.js Range первые байты: ${Array.from(head.slice(0,8)).map(b=>b.toString(16).padStart(2,'0')).join(' ')} → ${headInfo?.type || '?'}`);
                    }
                } catch(e){ console.log('⚠️ Range fail:', e.message); }
                
                // 📕 PDF / 🎵 аудио / 🎬 видео — качаем полностью
                if(headInfo && (headInfo.type==='pdf' || isMultimediaType(headInfo.type))){
                    return { ok:true, type:headInfo.type, needDownload:true };
                }
                
                // 📖 JSON — качаем полностью для парсинга
                if(headInfo && (headInfo.type==='json' || headInfo.type==='json-obj')){
                    const r = await fetchWithTimeout(chUrl, { credentials:'include' }, 15000);
                    if(!r.ok) return { ok:false };
                    const blob = await r.blob();
                    const text = await blob.text();
                    return { ok:true, type:headInfo.type, text };
                }
                
                // Fallback: качаем полностью
                const r = await fetchWithTimeout(chUrl, { credentials:'include' }, 20000);
                if(!r.ok) return { ok:false };
                const blob = await r.blob();
                const info = await detectBlobType(blob);
                console.log(`🔍 000.js полный: ${info.icon||''} ${info.name} (${fmtBytes(blob.size)})`);
                if(info.type==='pdf' || isMultimediaType(info.type)) return { ok:true, type:info.type, blob };
                if(info.type==='json' || info.type==='json-obj'){
                    const text = await blob.text();
                    return { ok:true, type:info.type, text };
                }
                return { ok:false };
            }catch(e){
        ...
        <div id="litres_downloader_ui">
            <div class="ldl-header">
                ...
                <div style="flex:1;min-width:0;">
                    ...
                    <div class="ldl-subtitle">v50.0 · multimedia 🎧🎬</div>
                </div>
    ...
    // 🎵 Сохранить аудио/видео как есть
    function saveMultimedia(blob, formatInfo){
        const safe = state.bookTitle.replace(/[\\/:*?"<>|]/g,'_').slice(0,100);
        const extMap = { 'MP3':'mp3','M4B':'m4b','M4A':'m4a','M4A/MP4':'m4a','FLAC':'flac','OGG':'ogg','WAV':'wav','MP4':'mp4','WEBM':'webm','MKV':'mkv' };
    ...
    async function startSmart(){
        ...
        try{
            ...
            // 🥇 Direct link (ZIP / аудио / видео)
            if(zipRes.ok){
                logStep('🥇 Direct link — скачиваем');
                state.isRunning = true; updateButtons();
                // Ожидаемые типы: ZIP + всё мультимедиа
                const expected = ['zip','audio-mp3','audio-m4b','audio-m4a','audio-flac','audio-ogg','audio-wav','video-mp4','video-webm'];
                ...
                if(blob && blob.size > 1024){
                    ...
                    const fn = m ? decodeURIComponent(m[1]) : `${state.bookTitle.replace(/[\\/:*?"<>|]/g,'_').slice(0,100)}.${info?.name?.toLowerCase()||'bin'}`;
                    triggerDownload(blob, fn);
                    zipInfo.style.display='inline';
            ...
            if(pdf000Res.ok && pdf000Res.needDownload){
                ...
                if(pdf000Res.type === 'pdf'){
                    logStep('🥈 PDF blob — качаем с прогрессом');
                    bookInfo.format = { icon:'📕', name:'PDF' }; updateFormatDisplay();
                    state.isRunning = true; updateButtons();
                    setStatus('📕 Скачиваем PDF...');
                    const pdfBlob = await downloadWithProgress(chUrl, { credentials:'include' }, 'PDF', ['pdf']);
                    if(!pdfBlob){ logWarn('PDF не скачался'); state.isRunning=false; updateButtons(); return; }
                    progressBar.style.width='100%'; percentText.textContent='100%';
                    setReadingStatus('📦 PDF получен, упаковываем...');
                    await finalizePdfToZip(pdfBlob);
                    state.isRunning = false; updateButtons(); return;
                }
                if(isMultimediaType(pdf000Res.type)){
                    logStep(`🥈 Мультимедиа: ${pdf000Res.type}`);
                    state.isRunning = true; updateButtons();
                    setStatus(`📥 Скачиваем ${pdf000Res.type}...`);
                    const mediaBlob = await downloadWithProgress(chUrl, { credentials:'include' }, 'Мультимедиа', [pdf000Res.type]);
                    if(!mediaBlob){ logWarn('Мультимедиа не скачалось'); state.isRunning=false; updateButtons(); return; }
                    const info = mediaBlob._detected || { icon:'🎵', name:'AUDIO' };
                    bookInfo.format = info; updateFormatDisplay();
                    saveMultimedia(mediaBlob, info);
                    state.isRunning = false; updateButtons(); return;
                }
            }

            // 🥈 000.js blob (если уже скачан)
            if(pdf000Res.ok && pdf000Res.blob){
                const info = await detectBlobType(pdf000Res.blob);
                if(info.type === 'pdf'){
                    logStep('🥈 PDF blob — упаковываем');
                    bookInfo.format = { icon:'📕', name:'PDF' }; updateFormatDisplay();
                    state.isRunning = true; updateButtons();
                    progressBar.style.width='100%'; percentText.textContent='100%';
                ...
                if(isMultimediaType(info.type)){
                    logStep(`🥈 Мультимедиа: ${info.name}`);
                    bookInfo.format = info; updateFormatDisplay();
                    state.isRunning = true; updateButtons();
                    progressBar.style.width='100%'; percentText.textContent='100%';
                    saveMultimedia(pdf000Res.blob, info);
                    state.isRunning = false; updateButtons(); return;
                }
            }
    ...
    window.downloaderUI = { version:'v50.0', start:startSmart, stop:stopDownload, state, Sound, addLog, strategyZip, strategy000js, strategyPdfjs, downloadWithProgress, detectBlobType, fetchBookInfo, fetchUserInfo, fetchJsonChapter, buildBookHtml, parseLitFile, litJsonToHtml, saveProgressToGitHub };
    ...
    async function init(){
        ...
        updateButtons();
        updateTabTitle();
        console.log('%c✅ LitRes Downloader v50.0 готов!', 'color:#4ade80;font-weight:bold;font-size:14px;');
    }
